 ...
 *					on the same network and a utility is looking for 
...
void AnnounceIP(void)
{
	...
	// Set the socket's destination to be a broadcast over our IP block
	...
	Remote.IPAddr.v[0] = AppConfig.MyIPAddr.v[0];
	Remote.IPAddr.v[1] = AppConfig.MyIPAddr.v[1];
	Remote.IPAddr.v[2] = AppConfig.MyIPAddr.v[2];
	Remote.IPAddr.v[3] = 255;
	...
	// router to a different portion of the network.
	UDPPut('M');
	UDPPut('A');
	UDPPut('C');
	UDPPut(' ');
	...
	// Convert the MAC address bytes to hex (text) before sending
	CharPtr = &AppConfig.MyMACAddr.v[0];
	for(i = 0; i < 5; i++)
	{
		MACChar = ((*CharPtr & 0xF0) >> 4)+ '0';
		if( MACChar > '9' )
			MACChar += 'A'-'9'-1;
		UDPPut(MACChar);
		MACChar = (*CharPtr++ & 0x0F) + '0';
		if( MACChar > '9' )
			MACChar += 'A'-'9'-1;
		UDPPut(MACChar);
		UDPPut('-');
	...
	// Send the last MAC address octet
	MACChar = ((*CharPtr & 0xF0) >> 4)+ '0';
	if( MACChar > '9' )
		MACChar += 'A'-'9'-1;
	UDPPut(MACChar);
