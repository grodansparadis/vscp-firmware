...
//#if !defined(MCHP_MAC)
//#error When using the Microchip ENC28J60, 'MCHP_MAC' should be defined in StackTsk.h
//#endif
...
#define	SC  ((0b111<<5) | 0x1F)	// System Command does not use an address.  It
...
void MACInit(void)
{
	 ...
	 */
	BankSel(MAADR0);
    WriteReg(MAADR0, MY_MAC_BYTE6);
    WriteReg(MAADR1, MY_MAC_BYTE5);
    WriteReg(MAADR2, MY_MAC_BYTE4);
    WriteReg(MAADR3, MY_MAC_BYTE3);
    WriteReg(MAADR4, MY_MAC_BYTE2);
    WriteReg(MAADR5, MY_MAC_BYTE1);
...
// Mark the received packet as being processed and free the buffer
...
BOOL MACGetHeader(MAC_ADDR *remote, BYTE* type)
{
	...
	if( !ReadETHReg(EIR).EIRbits.PKTIF )
		return FALSE;
	...
	if(header.NextPacketPointer > RXSTOP || 
	   header.NextPacketPointer < RXSTART ||
 ...
 * Note:            None
 *****************************************************************************/
#if 0 // TODO: Remove this line if your application uses this function
