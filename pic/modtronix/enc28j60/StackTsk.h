 ...
 * This value is specific to the Microchip Ethernet controllers.  
 * If a different Ethernet controller is used, this define is not
 * used.  When MAC_FILTER_BROADCASTS is defined, all broadcast 
 * packets that are received will be discarded by the hardware, 
 * except for ARP requests for our IP address.
 */
