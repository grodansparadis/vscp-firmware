...
namespace Xedia.Protocol.VSCP
{
	...
	public class CRCTool
	{
		...
		// 'refin' [0,1] specifies if a m_data byte is reflected before processing (UART) or not
		...
		/// </summary>.
		public long crctablefast (byte[] p, int length) 
		{
			if ((p == null) || (length > p.Length) || (crctab == null))
				return 0;
		...
		public long crcbitbybit(byte[] p) 
		{
		...
		public long crcbitbybitfast(byte[] p) 
		{
        ...
        #region subroutines
		private ulong reflect (ulong crc, byte bitnum) 
		{
