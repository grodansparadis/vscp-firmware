 ...
 * User must call for XEEWait() to ensure that write cycle is finished
 * before calling any other routine.
 *
 * @preCondition     XEEInit() && XEEBeginWrite() are already called.
 *
 * @return          XEE_SUCCESS if successful other value if failed.
...
{
    i2cGetByte();           // Read byte from I2C slave
    while( SSPCON2_RCEN );  // check that receive sequence is over.

    i2cPutAck();            //Put ACK bit
