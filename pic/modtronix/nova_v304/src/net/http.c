...
//Indicates to any client that the document will expire in 60 seconds. Can NOT use the cach contents after 60 seconds
    ...
    while( lbContinue )
    {
        ...
        case SM_HTTP_GET_TX_HDR:
            if ( TCPIsPutReady(ph->socket) )
            {
                ...
                //Send "Cache-Control: max-age=300, must-revalidate" for JavaScript files
                //Causes any javaScript file to be reloaded after 5 minutes
                /*
                if ((ph->var.get.fileType == HTTP_FILETYPE_JS) || (ph->var.get.fileType == HTTP_FILETYPE_ZJS)) {
                    sendRomStr(h, HTTPMSG_CACHECONTROL_MAXAGE300);
                    //sendLineEnd(h);     //"CRLF" characters is included in HTTPMSG_CACHECONTROL_MAXAGE60 string
                }
                */
