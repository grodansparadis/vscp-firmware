...
It is important to note that NO tag replacement will be done on the contents of compressed files. This
...
application must implement a callback function with an appropriate code. The Modtronix HTTP Server
does not perform "URL decoding". This means that if any of the form field text contains certain special
non-alphanumeric characters (such as <, >, #, %, etc.), the actual parameter value would contain "%xx"
("xx" being the two-digit hexadecimal value of the ASCII character) instead of the actual character.
For example, an entry of "<Name>" would return "%3CName%3C".
...
string. Only "*.cgi" files are parsed for these special tags.
<p>
To incorporate this real-time information, the corresponding CGI file (*.cgi) must contain a
...
@endif The HTTPGetVar() function will define strings that will replace tags found in the "*.cgi" file.
...
// IMPORTANT!!! This structure will be removed once HTTP Authentication has been
// implemented. Once this is done, user authentication will be read from HTTP header
// on receiving an HTTP command!
typedef struct _HTTP_USER {
    IP_ADDR ipAdr;  //IP Address of logged in user
    BYTE user;      //User type, is a HTTP_USER_XXX variable
} HTTP_USER;
 ...
 * is required (the parsed web pages contains a "\%nxx" tag) on any *.cgi pages, HTTPServer
 ...
 * <br>The <i>variable group</i> and <i>variable value</i> give the group and value of the tag
 ...
 * updated reference. If there are no more values left for this variable the application should send
 * HTTP_END_OF_VAR <b>and</b> set var to NULL. If there
 * are any bytes left to be sent, the application should return a value other than HTTP_START_OF_VAR or
 * HTTP_END_OF_VAR.
 *
 * @preCondition    HTTPInit() must already be called.
