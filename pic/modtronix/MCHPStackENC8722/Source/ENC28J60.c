 ...
 * is supplied to you for use solely and exclusively with Microchip 
 * products, as described by the license agreement accompanying this 
 * software. This software is owned by the Company and/or its supplier, 
...
void MACDiscardRx(void)
{
	...
	// high byte last.
	BankSel(ERXRDPTL);
	WriteReg(ERXRDPTL, LSB(NextPacketLocation));
	WriteReg(ERXRDPTH, MSB(NextPacketLocation));
}
...
void MACSetRxBuffer(WORD offset)
{
	...
	// Adjust the address to the desired location
	ReadPT.Val += sizeof(MCP_PREAMBLE);
	ReadPT.Val += offset;
...
WORD MACCalcRxChecksum(WORD offset, WORD len)
{
	...
	// Figure out where the beginning of the Ethernet packet is
	BankSel(ERXRDPTL);
 ...
 *
 * Note:            If a link partner is present, it will take 10s or maybe 
 *					even 100s of milliseconds before a new link will be 
 *					established after waking up.  While not linked, packets 
 *					which are transmitted will most likely be lost.  
 *					MACIsLinked() can be called to determine if a link is 
 *					established.
 *****************************************************************************/
