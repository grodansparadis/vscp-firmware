...
// Documentation
/**
@defgroup mod_examples_tick Tick Example
@ingroup  mod_examples

This example is contained in "./tick/src/main.c"

This example show how to use the 8-bit, 16-bit and 32-bit Tick functions.
********************************************************************/
...
void main(void)
{
    ...
    while(1)
    {
        ...
        if ( TickGetDiff16bit(t16Bit) >= ((TICK16)TICKS_PER_SECOND / (TICK16)2) )
        {
            t16Bit = TickGet16bit();
        ...
        if ( TickGetDiff8bit(t8Bit) >= ((TICK8)TICKS_PER_SECOND / (TICK16)2) )
        {
            t8Bit = TickGet8bit();
