...
void StackTask(void)
{
            ...
            if ( !MACGetHeader(&remoteNode.MACAddr, &type.MACFrameType) )
            {
                ...
                //debugPutRomString((ROM char*)"2");
                    ...
                    if (STACK_IS_DHCP_ENABLED)
                    {
                        ...
                        {
                            //debugPutGenMsg(2);     //@mxd:2:%s
                            //debugPutRomString((ROM char*)"3");

                            #if (DEBUG_STACKTSK >= LOG_INFO)
                            //debugPutMsg(1); //@mxd:1:DHCP Enabled but MAC not linked yet - set IP to 0.0.0.0
                            #endif
                            
                            //@TODO Check if this is required? If stackFlags.bits.bInConfigMode is not checked,
                            //then our IP will be reset to 0.0.0.0 each time the link is lost. Is this correct?
    ...
    //debugPutGenMsg(2);     //@mxd:2:%s
    //debugPutRomString((ROM char*)"MACTask");
