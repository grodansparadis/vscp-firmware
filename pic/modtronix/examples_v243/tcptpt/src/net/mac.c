...
#define CURRP   0x07            /* RX Buffer's page PUT pointer rd and wr */
...
#define MAX_DATA_BUFFERS        ((TXPAGES*NIC_PAGE_SIZE)/MAC_TX_BUFFER_SIZE)
...
//will be cleared once the transmission is finished, and this buffer is available again. Is 0xff when none
BYTE currTXingBuffer;
...
void    MACInit(void)
{
    ...
    MACCurrTxbuf = 0;
    currTXingBuffer = 0xff;     //There is current no buffer being transmitted!
...
BOOL MACIsTxReady(void)
{
    ...
    if (MACCurrTxbuf == currTXingBuffer) {
        //Transmission is finished
        if ((NICGet(CMDR) & 0x04) == 0) {
            currTXingBuffer = 0xff; //Indicate that there is currently no buffer being transmitted
            
            return TxBuffers[MACCurrTxbuf].Flags.bFree;
        }
...
void MACFlush(void)
{
    ...
    currTXingBuffer = MACCurrTxbuf;     //Mark the buffer that is currently being transmitted
    ...
    TxBuffers[MACCurrTxbuf].Flags.bFree = TRUE;
    ...
    #if (MAX_DATA_BUFFERS > 1)
        //Only execute this code if there is more then 1 transmit buffer, else MACCurrTxbuf is always the
        //same, and no need to do this test!

        for ( i = 0; i < MAX_DATA_BUFFERS; i++ )
        {
            if ( TxBuffers[i].Flags.bFree && (currTXingBuffer != i))
            {
