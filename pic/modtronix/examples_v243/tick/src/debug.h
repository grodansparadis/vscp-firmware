...
Each debug module has to be configued for it's desired debug level. For example, if detailed debug
...
In the source code debug inforation is written out with lines that look like this:
@code
...
is configued (in the projdefs.h file) to be LOG_INFO or higher (LOG_ERROR, LOG_WARN or LOG_INFO).

After the project has been configured for the desired debug level, it has to be recompiled and the
target board programed with the new firmware.
...
received from the hardware being debugged. These detailed messages are defined in a project file. The @appname
is shipped with such a project file, and it is <i>"../src/debug.xml"</i>.

At delivery, the @appname is configured to output Error and Warning debug information only. For details on
changing the debug information outputtted, or disabling it, see the @ref mod_sys_debug section of this document.

To view the debug information outputted by the @appname, do the following:
- Download, install and start the @ref tools_mxd
- Load the debug project file included in the source code - <i>"../src/debug.xml"</i>
- Connect the target board to the serial port of the PC running the @ref tools_mxd.
...
********************************************************/
// !!!!!!!!!! DEBUGGING NOT USED FOR THIS PROJECT !!!!!!!!!!
//#include "serbasic.h"
//#include "serint.h"
