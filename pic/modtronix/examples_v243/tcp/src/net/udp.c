 ...
 *                      is returned.
 */
UDP_SOCKET UDPOpen(WORD localPort,
...
BOOL UDPGet(BYTE *v)
{
    ...
    *v = MACRxbufGet(); //Read a single byte from the current MAC Receive buffer
...
WORD UDPGetArray(BYTE *buffer, WORD count)
{
    ...
    bytesRead = MACRxbufGetArray(buffer, count);
...
BOOL UDPProcess(NODE_INFO *remoteNode, IP_ADDR *destIP, WORD len)
{
    ...
    MACRxbufGetArray((BYTE*)&h, sizeof(h));
    ...
    {
        //If there is no matching socket, There is no one to handle this data.  Discard it.
        MACRxbufDiscard();
    }
    else
    {
