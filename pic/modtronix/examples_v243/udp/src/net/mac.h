 ...
 *
 * @return Handle of current receive buffer. Is a number from 0-255.
 */
#define MACGetRxBuffer()        (MACCurrRxbuf)

/**
 * Get handle of current transmit buffer. Is a number from 0-255. The current transmit
 * buffer will automatically be set to the next available buffer after a MACFlush() function.
 *
 * @return Handle of current transmit buffer. Is a number from 0-255.
 */
#define MACGetTxBuffer()        (MACCurrTxbuf)

#if     !defined(THIS_IS_MAC_LAYER)
#if     !defined(STACK_USE_SLIP)
extern  BYTE MACCurrTxbuf;
extern  BYTE MACCurrRxbuf;
#else
#define MACCurrTxbuf      (0)
#define MACCurrRxbuf      (0)
#endif
#endif
