 ...
 *
 * ****************************************************************************
 * $Id: serint.c 835 2008-02-08 13:58:32Z jaguarondi $
 */

/*
 * Interrupt driven Serial receive and transmit handler.
 *
 * Contains a receive and transmit buffer. All bytes received via the USART are
 * placed in the receive buffer. All bytes that have to be transmitted, are
...
void serInit( void )
{
	...
	RCSTA = 0b10010000; 	// USART Enable, 8 bits, RX enable,
...
// This function should be called from the ISR if "(TXIF && TXIE)" evaluates to
// true.
//

void serTxIsr( void )
{
	// Transmit next byte contained in transmit buffer. If whole buffer has
	// been transmitted, disable transmit interrupts.
	if ( idxSerTxbufPut != idxSerTxbufGet ) {
		TXREG = txBuf[ idxSerTxbufGet ];
		idxSerTxbufGet = ( ++idxSerTxbufGet ) & SER_TXBUF_MASK;
	}
	else {
		PIE1_TXIE = 0;	// Disable USART Transmit interrupt when nothing more
		// to TX
	}
}
...
void serPutByte( BYTE c )
{
	...
	// Check if buffer is full. If this is the case, wait until a byte is
	// transmitted by the interrupt routine and buffer has place again.
	...
	// We are currently trsanmitting. This means that the TXIF will soon be set
	// after the current byte has been transmitted, and the TX buffer will be
	// checked for any unsent data. Add current byte to TX buffer.
	else {
		// Seeing that an TX interrupt can happen any time during this
		// function, only update idxTxbufPut at the end of the routine
