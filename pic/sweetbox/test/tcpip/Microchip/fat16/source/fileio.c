 ...
 * ****************************************************************************
 * 
 * $Id: FileIO.c,v 1.3 2006/10/24 16:37:51 C12923 Exp $
 *
 * $Name:  $
 *
 *****************************************************************************/
    ...
    if( ForceRead || !(*curEntry & 0xf) ) {   // Only 16 entries per sector
       ...
       if( ccls < LAST_CLUSTER ) {
          ...
          if( !ccls && ((sector + offset2) >= dsk->data) ) {
             ...
             if( !MediaReadSector( sector + offset2, dsk->buffer ) )
                dir = (DIRENTRY)NULL;
             else {
...
PRIVATE BOOL WriteFileEntry( FILEOBJ fo, WORD *curEntry )
{
    ...
    // Now write it
    if( !MediaWriteSector( sector + offset2, dsk->buffer ) )
       status = FALSE;
    else
       status = TRUE;
...
PRIVATE CETYPE __fread( FSFILE *fo, void *pDest, WORD count, WORD *pBytesRead )
{
    ...
    *pBytesRead = 0;
    ...
    if( (gBufferOwner != fo) && (pos != MEDIA_SECTOR_SIZE) ) {
       ...
       if( !MediaReadSector( sec_sel, dsk->buffer ) )
    ...
    while( count ) {
        ...
        *(BYTE *)pDest = RAMread( dsk->buffer, pos++ );
        pDest++;
        seek++;
        (*pBytesRead)++;
        count--;
...
PRIVATE CETYPE __fseek( FILEOBJ fo, DWORD offset, BYTE origin )
{
    ...
    if( !MediaReadSector( temp, dsk->buffer ) )
 ...
 * Note:           
 ********************************************************************/
PUBLIC FSFILE *GetRootDirectory(void)
{
    FSFILE *filePtr;
    FSFILE *pFile;
    ...
    if( !InitFileObject( filePtr, "PM3CARD:\\*" ) )
    {
        FreeFilePtr( filePtr );
        return NULL;
    }
