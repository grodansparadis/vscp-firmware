...
static BYTE gblSDMMCBuf[MEDIA_SECTOR_SIZE];
DISK gblDiskData;
 ...
 *****************************************************************************/
static CETYPE LoadMBR( DISK *dsk )
{
    ...
    if( !MediaReadSector( FO_MBR, dsk->buffer ) )
 ...
 * Note:            Should not be called by user
 *****************************************************************************/
static CETYPE LoadBootSector( DISK *dsk )
{
    WORD RootDirSectors;
    ...
    // Get the Boot sector
    if( !MediaReadSector( dsk->firsts, dsk->buffer ) )
       error = CE_BAD_SECTOR_READ;
       ...
       if( (BootSec->Signature0 != FAT_GOOD_SIGN_0) ||
          ...
          TotSec = (DWORD)ReadWord( dsk->buffer, BSI_TotSec16 );
          if( !TotSec )
          ...
          dsk->fatcopy = ReadByte( dsk->buffer, BSI_NumFATs );   // byte
 ...
 * Note:            Should not be called by user
 *****************************************************************************/
static CETYPE DiskMount( DISK *dsk, BYTE *b, MMCSTATE *Flag )
{
    ...
    Flag->isWP = MediaIsWriteProtected();
    Flag->isSDMMC = TRUE;
...
PUBLIC BOOL FATMediaInit( void )
{
    ...
    if( MediaInit( &card_status ) != TRUE )
       return( FALSE );

    if( DiskMount( &gblDiskData, buffer, &Flag ) != CE_GOOD )
       return( FALSE );
...
PUBLIC WORD FATWrite( DISK *dsk, WORD cls, WORD v )
{
    ...
    l = dsk->fat + (p >> 9);
    p &= 0x1ff;
    if( !MediaReadSector( l, dsk->buffer ) )
       return FAIL;
    ...
    for( i = 0, li = l; i < dsk->fatcopy; i++, li += dsk->fatsize )
       if( !MediaWriteSector( l, dsk->buffer) )
          return FAIL;
...
PUBLIC CETYPE EraseCluster( DISK *disk, WORD cluster )
{
    ...
    for( index = 0; (index < disk->SecPerClus) &&
                    (error == CE_GOOD); index++ ) {
       if( !MediaWriteSector( SectorAddress++, disk->buffer ) )
