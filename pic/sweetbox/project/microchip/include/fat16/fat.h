 ...
 * ****************************************************************************
 *
 * $Id: fat.h 161 2008-10-15 19:30:28Z akhe $
 *
 * $Name:  $
 *
 *****************************************************************************/
...
typedef enum _CETYPE
{
    ...
    CE_NO_DISK_BUF,       // no disk buffer 
    CE_NO_SECTOR_BUF      // no sector buffer
} CETYPE;
...
} __attribute__((__packed__)) _BPB_FAT32;
...
typedef struct __BPB_FAT16 {
    ...
    BYTE BS_VolLab[11];      // Volume Label
    BYTE BS_FilSysType[8];   // File system type, not used for determination
} __attribute__((__packed__)) _BPB_FAT16;
...
typedef struct __BPB_FAT12 {
    ...
    BYTE BS_VolLab[11];      // Volume Label
    BYTE BS_FilSysType[8];   // File system type, not used for determination
} __attribute__((__packed__)) _BPB_FAT12;
...
} __attribute__((__packed__)) PTE_FAT;
...
} __attribute__((__packed__)) _PT_FAT;
...
typedef struct _BSI_SECTOR
{
    ...
    BYTE    vol_lab[11];
    DWORD   fil_sys_type;
} __attribute__ ((__packed__)) BSI_SECTOR;
...
} __attribute__((__packed__)) _DIRENTRY;
...
} __attribute__((__packed__)) _LDIR;
...
PUBLIC CETYPE DiskMount( UINT idx, DISK *dsk, BYTE *b );
