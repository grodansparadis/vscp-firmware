...
(USB_PRINTER_COMMAND).  The exact values to send for each bar code type can
vary for the particular POS printer, and not all printers support all bar code
types.  Be sure to test the output on the target printer, and adjust the values
specified in usb_host_printer_esc_pos.c as necessary.  Refer to the printer's
technical documentation for the required values.  Do not alter this enumeration.
*/
#ifdef USB_PRINTER_POS_BARCODE_SUPPORT
typedef enum
{
    ...
    USB_PRINTER_POS_BARCODE_ITF,
        ...
        // and '+' (plus).
        // The check digit will be automatically calculated and appended.
    USB_PRINTER_POS_BARCODE_CODEABAR,
