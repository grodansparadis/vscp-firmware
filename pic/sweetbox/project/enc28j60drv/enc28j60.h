 ...
 *
 * $Id: enc28j60.h 276 2008-12-29 11:45:04Z akhe $
 *
 ********************************************************************/
...
#define MAX_ETHERNET_FRAME_SZ           1518    // from ether.h                
...
struct NetPacket
{
	...
	uint8_t*	pNetworkLayer;
	uint8_t*	pMacLayer;

	uint8_t*	pSrcMAC; 	//valid only for Rx Packets
	uint8_t*	pSrcIP;		//valid only for Rx Packets
	uint8_t*	pDestIP;	//valid only for Rx Packets
	struct 	    NetPacket *pTcpNext;
	struct      MCSocket * pSocket;
	uint8_t*	pData;

	uint32_t    TCPACKNum;
	uint32_t	TimeStamp;	    // Time when rcvd or queued for xmit TICK
	uint32_t	ARPTickStart;   // TICK
	BOOL	    TxAutoDeAlloc;
	uint16_t	PktLen;
	uint8_t	    PktType;	//valid only for Rx Packets
...
/*!
    ...
    \param pnetif Pointer ot lwIP network interface structure
    
    MACInit sets up the PIC's SPI module and all the 
...
/*!
    Get packet from ethernet controller
    \return length of packet or zeror if not pacjet was read. 
*/
uint16_t enc28j60_MACGetRxPacket( uint8_t *pBuf );

/*!
    Write a packet
    \param pBuf Pointer to the packet.
...
*/
//uint16_t enc28j60_MACGetTxQueueCount( void );
