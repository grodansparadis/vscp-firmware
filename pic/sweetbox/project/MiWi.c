...
extern ROM uint8_t availableChannels[AVAILABLE_CHANNELS_SIZE];
	...
	BOOL DataEncrypt(IOPUT uint8_t *Input, IOPUT uint8_t *DataLen, INPUT uint8_t *Header, INPUT uint8_t HeaderLen);
	BOOL DataDecrypt(IOPUT uint8_t *Input, IOPUT uint8_t *DataLen, INPUT uint8_t *Header, INPUT uint8_t HeaderLen);
...
uint8_t currentSearchChannel;
uint8_t IEEESeqNum;
uint8_t MiWiSeqNum;

#if defined(I_AM_COORDINATOR_CAPABLE)
uint8_t indirectBuffer[INDIRECT_BUFFER_SIZE];
...
WORD_VAL myPANID;
volatile uint8_t failureCounter = 0;
MIWI_STATE_MACHINE MiWiStateMachine;
uint8_t currentChannel;
uint8_t myParent;


// Prototype to ensure the vector is  correctly installed. 
void __attribute__( (interrupt( ipl2 ), vector( _INT2_VECTOR ) )) vMRF24J50ISR_Wrapper( void );
...
ThisPacketIsForMe:
    							...
    							{
	    							uint8_t DataLen = RxBuffer[0] - i - 14;
                                ...
                                {                                
                                    ...
                                    case OPEN_P2P_SOCKET_REQUEST:
                                        ...
                                        else
                                        {
                                            ...
                                            openSocketInfo.socketStart = timeBaseGet32();
            ...
            case FRAME_TYPE_BEACON:
                {
                    ...
                    #if defined(I_AM_COORDINATOR_CAPABLE)
                    {
                        ...
                        uint8_t coordinatorNumber = RxBuffer[7] & 0x03;
...
DiscardPacketAndFinish:
    ...
    tickCurrentTime = timeBaseGet32();
    tickDifference = tickCurrentTime - startTick;
    ...
    {       
        ...
        if(currentSearchChannel<AVAILABLE_CHANNELS_SIZE)    
        {
            ...
            if(tickDifference > NETWORK_DISCOVERY_TIMEOUT)
            {
                //start the next channel
                startTick = timeBaseGet32();
    ...
    else if(MiWiStateMachine.attemptingToJoin == 1)
    {
        ...
        if ( tickDifference > aResponseWaitTime )
        {
    ...
    else
    {
        ...
        while(1)
        {
            ...
            tickDifference = indirectBuffer[ i + 5 ] +
                                ( indirectBuffer[ i + 4 ] >> 8 ) +
                                ( indirectBuffer[ i + 3 ] >> 16 ) +
                                ( indirectBuffer[ i + 2 ] >> 24 );
            
            tickDifference = tickCurrentTime - tickDifference;
            
            if(tickDifference < INDIRECT_BUFFER_TIMEOUT)
            {
                break;
            }    
    ...
    #else //P2P_ONLY
    {
        ...
        if(openSocketInfo.status.bits.requestIsOpen)
        {
            ...
            {
                tickDifference = tickCurrentTime - openSocketInfo.socketStart;
                
                if(tickDifference > OPEN_CLUSTER_SOCKET_TIMEOUT)
                {
...
/*********************************************************************
 * Function:        uint8_t SendReportByShortAddress(WORD_VAL PANID, WORD_VAL ShortAddress)
 *
 ...
 * Output:          uint8_t the MiWi handle for this packet
 *
...
uint8_t SendReportByShortAddress(WORD_VAL PANID, WORD_VAL ShortAddress, BOOL forwardPacket)
{
    ...
    else
    {
            ...
            {
                uint8_t parentNode = tempShortAddress.v[1] & 0x03;
                if(RoutingTable[parentNode] != 0)
                {
                    ...
                    {
                        uint8_t randomNumber = 1<<(TMRL & 0x03); //not really random but good enough
                        uint8_t j;
...
/*********************************************************************
 * Function:        uint8_t SendReportByLongAddress(uint8_t *pLongAddress)
 *
 * PreCondition:    TxBuffer is loaded with the packet that needs to be
 *                  sent
 *
 * Input:           uint8_t *pLongAddress
 * Output:          uint8_t the MiWi application handle for this packet
 *
 * Side Effects:    The TxBuffer gets transmited and the Tx pointer 
 *                  gets reset
 *
...
/*********************************************************************
 * Function:        uint8_t SendReportByHandle(uint8_t handle)
 *
 * PreCondition:    TxBuffer is loaded with the packet that needs to be
 *                  sent
 *
 * Input:           uint8_t handle - the handle of the device that you are
...
/*********************************************************************
 * Function:        void SendIndirectPacket(uint8_t handle,uint8_t dataRequestType)
 *
 * PreCondition:    TxBuffer is loaded with the packet that needs to be
 *                  sent
 *
 * Input:           uint8_t handle - the entry in the neighbor table that
 *                      applies to this device
 *                  uint8_t dataRequestType - this value needs to be either
...
/*********************************************************************
 * Function:        void PHYSetLongRAMAddr(uint16_t address, uint8_t value)
 *
 * PreCondition:    Communication port to the MRF24J40 initialized
 *
 * Input:           uint16_t address is the address of the LONG RAM address
 *                      that you want to write to
 *                  uint8_t value is the value that you want to write to
 *                      that register
 *
 * Output:          None
 *
...
void PHYSetLongRAMAddr(uint16_t address, uint8_t value)
{
    ...
    mSetMiWiCSOn();
    Nop();
    putcSPI((((uint8_t)(address>>3))&0x7F)|0x80);
    putcSPI((((uint8_t)(address<<5))&0xE0)|0x10);
    putcSPI(value);
    Nop();
    mSetMiWiCSOff();
    RFIE = tmpRFIE;
...
/*********************************************************************
 * Function:        void PHYSetShortRAMAddr(uint8_t address, uint8_t value)
 *
 * PreCondition:    Communication port to the MRF24J40 initialized
 *
 * Input:           uint8_t address is the address of the short RAM address
...
/*********************************************************************
 * Function:        void PHYGetShortRAMAddr(uint8_t address, uint8_t value)
 *
 * PreCondition:    Communication port to the MRF24J40 initialized
 *
 * Input:           uint8_t address is the address of the short RAM address
 ...
 *                      include file.
 *
 * Output:          uint8_t the value read from the specified short register
 *
 * Side Effects:    None
 *
...
/*********************************************************************
 * Function:        uint8_t PHYGetLongRAMAddr(uint16_t address)
 *
 * PreCondition:    Communication port to the MRF24J40 initialized
 *
 * Input:           uint16_t address is the address of the long RAM address
 ...
 *
 * Output:          uint8_t the value read from the specified Long register
 *
 * Side Effects:    None
 *
...
/*********************************************************************
 * Function:        void ClearNetworkTable(uint8_t options)
 *
 * PreCondition:    None
 *
 * Input:           uint8_t options - if the following option bits are set
...
/*********************************************************************
 * Function:        void FormNetwork(uint16_t PANID)
 *
 * PreCondition:    None
 *
 * Input:           uint16_t PANID - the # of the PANID that needs to be created
...
/*********************************************************************
 * Function:        void OpenSocket(uint8_t socketType)
 *
 * PreCondition:    None
 *
...
/*********************************************************************
 * Function:        void JoinNetwork(uint8_t handle)
 *
 * PreCondition:    None
 *
 * Input:           uint8_t handle - the index into the network table of 
...
/*********************************************************************
 * Function:        BOOL SendIndirectMessage(uint8_t handle)
 *
...
BOOL SendIndirectMessage(uint8_t handle)
{
    ...
    if((TxData + 6 + (TX_BUFFER_SIZE - TxHeader - 1)) < (INDIRECT_BUFFER_SIZE - i))
    {
        ...
        indirectBuffer[i++] = ( t >> 24 ) & 0xff; // t.byte.b3;
        indirectBuffer[i++] = ( t >> 16 ) & 0xff; // t.byte.b2;
        indirectBuffer[i++] = ( t >> 8 ) & 0xff;  // t.byte.b1;
        indirectBuffer[i++] = t & 0xff;           // t.byte.b0;
...
/*********************************************************************
 * Function:        void SetChannel(uint8_t channel)
 *
 * PreCondition:    MRF24J40 is initialized
 *
 * Input:           uint8_t channel - this is the channel that you wish
...
/*********************************************************************
 * Function:        uint8_t SearchForShortAddress(uint8_t entryType)
 *
...
void MRF24J40Wake(void)
{
    ...
    failureTimer = timeBaseGet32();

    while ( 1 )
    {       
        currentDifference = timeBaseGet32() - failureTimer;
        
        if ( currentDifference > 6250 )
        {
            break;
        }
...
/*********************************************************************
 * Function:        void vMRF24J40ISR_Handler(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 *                  MiWi stack. The mrf24j40_isr_wrapper.S wrapps this
 *                  routine in a secure way.
 ********************************************************************/

void vMRF24J40ISR_Handler( void )
{
    if ( RFIE && RFIF )
    {    
...
/*********************************************************************
 * Function:        CIPHER_STATUS PHYCipher(INPUT CIPHER_MODE CipherMode, INPUT SECURITY_INPUT SecurityInput, OUTPUT uint8_t *OutData, OUTPUT uint8_t *OutDataLen)
 *
 ...
 *
 * Output:          uint8_t *OutData                - Encrypted or decrypted data, including MIC
 *                  uint8_t *OutDataLen             - Data length after cipher operation, including MIC bytes
 ...
 ********************************************************************/
CIPHER_STATUS PHYCipher(INPUT CIPHER_MODE CipherMode, INPUT SECURITY_INPUT SecurityInput, OUTPUT uint8_t *OutData, OUTPUT uint8_t *OutDataLen)
{
	uint8_t CipherRetry = CIPHER_RETRY;
	uint8_t i;
	uint16_t loc;
...
/*********************************************************************
 * Function:        BOOL DataEncrypt(IOPUT uint8_t *Input, IOPUT uint8_t *DataLen, INPUT uint8_t *Header, INPUT uint8_t HeaderLen)
 *
 * PreCondition:    Input and Header has been filled 
 *
 * Input:           uint8_t *Header   - Point to MiWi header
 *                  uint8_t HeaderLen - MiWi header length
 *
 * Output:          BOOL           - If data encryption successful
 *
 * Input/Output:    uint8_t *Input    - Pointer to the data to be encrypted. The encrypted data will be write back to the pointer
 *                  uint8_t *DataLen  - Input as the length of the data to be encrypted. The encrypted data length (including MICs) will be written back
 *
...
/*********************************************************************
 * Function:        BOOL DataDecrypt(IOPUT uint8_t *Input, IOPUT uint8_t *DataLen, INPUT uint8_t *Header, INPUT uint8_t HeaderLen)
 *
 * PreCondition:    Input and Header has been filled 
 *
 * Input:           uint8_t *Header   - Point to MiWi header
 ...
 * Output:          BOOL           - If data encryption successful
 *
 * Input/Output:    uint8_t *Input    - Pointer to the data to be decrypted. The decrypted data will be write back to the pointer
 *                  uint8_t *DataLen  - Input as the length of the data to be decrypted. The encrypted data length (excluding MICs) will be written back
 *
