...
const uint8_t vscp_deviceURL[] = "vscphost/DIV4.xml";
...
void main()
{
	...
	while ( 1 ) {           // Loop Forever	
		...
		if ( ten_ms_clock > 10 ) {
            ...
            bvalues[5] = BUTTON6;
            bvalues[6] = BUTTON7;
            bvalues[7] = 0;   // BUTTON8 not connected
            bvalues[8] = BUTTON9;
            bvalues[9] = BUTTON10;
            bvalues[10] = BUTTON11;
            bvalues[11] = BUTTON12;
...
void init_app_eeprom( void )
{
	...
	for ( i=0; i<15; i++ ) {
		writeEEPROM( VSCP_EEPROM_END + REG_BUTTON1_SUBZONE + i, 0 );
		writeEEPROM( VSCP_EEPROM_END + REG_BUTTON1_ZONE + i, 0 );
		writeEEPROM( VSCP_EEPROM_END + REG_BUTTON_TIMEOUT1 + i, 0 );
		writeEEPROM( VSCP_EEPROM_END + REG_BUTTON_CONTROL1 + i, BUTTON_CONTROLBIT_INVERT);
	}
...
uint8_t vscp_readAppReg( uint8_t reg )
{
	...
	if ( reg == REG_BUTTON_STATUS1_7 ) {
		rv = BUTTON1_7 & 0b01111111;
	}
	else if ( reg == REG_BUTTON_STATUS9_15 ) {
		rv = (BUTTON9_14 & 0b00111111) | (BUTTON15 << 6);
	}
...
void SendEvent( unsigned char code, unsigned char eventClass, unsigned char eventTypeId, unsigned char msb1, unsigned char lsb1, unsigned char msb2, unsigned char lsb2 ) 
{
	...
	vscp_omsg.data[ 1 ] = readEEPROM( VSCP_EEPROM_END + REG_BUTTON1_ZONE + lsb1);
...
// 
// The routine expects the index of the button that is timed-out
//
// REMARK :
// This doDM is (mis)used to select an event which is send when a button is timed-out
// This means the some DM-atributes are used and some don't.
// Used attributes :
// VSCP_DM_FLAG_ENABLED : Enable/disable the DM-rule.
// VSCP_DM_FLAG_CHECK_OADDR : When true, the oaddr must be tesame as the timed-out button index
//                            Otherwise the event will be send for each timed-out button.
// VSCP_DM_FLAG_CHECK_ZONE : When true, the transmitted zone will be 255. 
// 							Otherwise the button zone will be used.
//							And if this is 0 then the module zone will be used.
// VSCP_DM_FLAG_CHECK_SUBZONE : When true, the transmitted subzone will be 255. 
// 								Otherwise the button subzone will be used.
//								And if this is 0 then the module subzone will be used.
// VSCP_DM_POS_CLASSFILTER : The class-event which will be send.
// VSCP_DM_POS_TYPEFILTER : The type-event which will be send.
// VSCP_DM_POS_ACTIONPARAM : Send as reserved byte

void doDM( unsigned char idx )
{
	unsigned char i;
	...
	for ( i=0; i<DESCION_MATRIX_ELEMENTS; i++ ) {
		...
		if ( dmflags & VSCP_DM_FLAG_ENABLED ) {
			...
			if ( ( dmflags & VSCP_DM_FLAG_CHECK_OADDR ) &&  
                 (  idx != readEEPROM( VSCP_EEPROM_END + REG_DESCION_MATRIX + ( 8 * i ) ) ) ) {  
                continue;
			}
			
			class = readEEPROM( VSCP_EEPROM_END + REG_DESCION_MATRIX +	( 8 * i ) + VSCP_DM_POS_CLASSFILTER );
			type = readEEPROM( VSCP_EEPROM_END + REG_DESCION_MATRIX + ( 8 * i ) + VSCP_DM_POS_TYPEFILTER );
				
			vscp_omsg.priority = VSCP_PRIORITY_MEDIUM;
			vscp_omsg.flags = VSCP_VALID_MSG + 7;
			vscp_omsg.class = class;
			vscp_omsg.type = type;
		
			vscp_omsg.data[ 0 ] = readEEPROM( VSCP_EEPROM_END + REG_DESCION_MATRIX + ( 8 * i ) + VSCP_DM_POS_ACTIONPARAM );
			if ( dmflags & VSCP_DM_FLAG_CHECK_ZONE  ) { 
				vscp_omsg.data[ 1 ] = 255;
			}
			else {
				vscp_omsg.data[ 1 ] = readEEPROM( VSCP_EEPROM_END + REG_BUTTON1_ZONE + idx);
			}
		    if ( vscp_omsg.data[ 1 ] == 0x00 ) {
				vscp_omsg.data[ 1 ] = readEEPROM( VSCP_EEPROM_END + EEPROM_ZONE );
		    }
			if ( dmflags & VSCP_DM_FLAG_CHECK_SUBZONE  ) { 
				vscp_omsg.data[ 2 ] = 255;
			}
			else {
				vscp_omsg.data[ 2 ] = readEEPROM( VSCP_EEPROM_END + REG_BUTTON1_SUBZONE + idx);
			}
		    if ( vscp_omsg.data[ 2 ] == 0x00 ) {
				vscp_omsg.data[ 2 ] = readEEPROM( VSCP_EEPROM_END + EEPROM_SUBZONE );
		    }
			vscp_omsg.data[ 3 ] = 0;
			vscp_omsg.data[ 4 ] = idx;
			vscp_omsg.data[ 5 ] = 0;
			vscp_omsg.data[ 6 ] = 0;
