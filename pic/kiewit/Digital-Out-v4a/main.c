...
const uint8_t vscp_deviceURL[] = "vscphost/DOV4a.xml";
...
unsigned char relay_pulse_timer[8];
unsigned char relay_protection_timer[8];
...
void init()
{
	...
	// RC4 - Output - Relay 5
	// RC5 - Output	- Relay 6 
	// RC6 - Output - TX - Relay 7
	// RC7 - Input  - RX - Relay 8
	TRISC = 0b00000000;
...
void init_app_eeprom( void )
{
	...
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL7, RELAY_CONTROLBIT_ONEVENT | 
										RELAY_CONTROLBIT_OFFEVENT |
										RELAY_CONTROLBIT_ENABLED );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL8, RELAY_CONTROLBIT_ONEVENT | 
										RELAY_CONTROLBIT_OFFEVENT |
										RELAY_CONTROLBIT_ENABLED );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL9, RELAY_CONTROLBIT_ONEVENT | 
										RELAY_CONTROLBIT_OFFEVENT |
										RELAY_CONTROLBIT_ENABLED );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL10, RELAY_CONTROLBIT_ONEVENT | 
										RELAY_CONTROLBIT_OFFEVENT |
										RELAY_CONTROLBIT_ENABLED );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL11, RELAY_CONTROLBIT_ONEVENT | 
										RELAY_CONTROLBIT_OFFEVENT |
										RELAY_CONTROLBIT_ENABLED );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL12, RELAY_CONTROLBIT_ONEVENT | 
										RELAY_CONTROLBIT_OFFEVENT |
										RELAY_CONTROLBIT_ENABLED );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL13, RELAY_CONTROLBIT_ONEVENT | 
										RELAY_CONTROLBIT_OFFEVENT |
										RELAY_CONTROLBIT_ENABLED );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL14, RELAY_CONTROLBIT_ONEVENT | 
										RELAY_CONTROLBIT_OFFEVENT |
										RELAY_CONTROLBIT_ENABLED );

	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME0, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME1, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME2, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME3, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME4, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME5, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME6, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME7, 0 );
	
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME0, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME1, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME2, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME3, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME4, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME5, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME6, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME7, 0 );
	...
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY9_SUBZONE, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY10_SUBZONE, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY11_SUBZONE, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY12_SUBZONE, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY13_SUBZONE, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY14_SUBZONE, 0 );
	writeEEPROM( VSCP_EEPROM_END + REG_RELAY15_SUBZONE, 0 );
	...
	if ( relay_pulse_timer[ i ] ) {
		...
		if ( !relay_pulse_timer[ i ] ) {
		    ...
		    switch ( i ) {
				...
				// Reload timer
				relay_pulse_timer[ i ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME0 + i );
...
uint8_t vscp_readAppReg( uint8_t reg )
{
	...
	if ( ( reg >= REG_RELAY_STATE0 ) && ( reg <= REG_RELAY_STATE14 ) ) {
		...
		switch ( reg ) {
			...
			case REG_RELAY_STATE7:
				rv = OUT8;
				break;
			case REG_RELAY_STATE8:
				rv = OUT9;
				break;
			case REG_RELAY_STATE9:
				rv = OUT10;
				break;
			case REG_RELAY_STATE10:
				rv = OUT11;
				break;
			case REG_RELAY_STATE11:
				rv = OUT12;
				break;
			case REG_RELAY_STATE12:
				rv = OUT13;
				break;
			case REG_RELAY_STATE13:
				rv = OUT14;
				break;
			case REG_RELAY_STATE14:
				rv = OUT15;
				break;
		}
	}
			...
			switch ( reg ) {
				...
				case REG_RELAY_STATE0:
					...
					if ( 1 == val ) {
						...
						relay_pulse_timer[ 0 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME0 );					
					}
					break;
				...
				case REG_RELAY_STATE1:
					...
					if ( 1 == val ) {
						...
						relay_pulse_timer[ 1 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME1 );
					}
					break;
				...
				case REG_RELAY_STATE2:
					...
					if ( 1 == val ) {
						...
						relay_pulse_timer[ 2 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME2 );
					}
					break;
				...
				case REG_RELAY_STATE3:
					...
					if ( 1 == val ) {
						...
						relay_pulse_timer[ 3 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME3 );
					}
					break;
				...
				case REG_RELAY_STATE4:
					...
					if ( 1 == val ) {
						...
						relay_pulse_timer[ 4 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME4 );
					}
					break;
				...
				case REG_RELAY_STATE5:
					...
					if ( 1 == val ) {
						...
						relay_pulse_timer[ 5 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME5 );
					}
					break;
				// Relay 6 Control
				case REG_RELAY_STATE6:
					...
					if ( 1 == val ) {
						...
						if( readEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL5 ) & RELAY_CONTROLBIT_PROTECTION ) {
							relay_protection_timer[ 5 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME6 );
						}
						...
						OUT7 = 0;			// Start out at a known state
						rv = 2;
						bOn = FALSE;
						relay_pulse_flags |= 0x40;	// Enable pulse output	
						
						relay_pulse_timer[ 5 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME6 );
					}
					break;
				// Relay 7 Control
				case REG_RELAY_STATE7:
					...
					if ( 1 == val ) {
						...
						if( readEEPROM( VSCP_EEPROM_END + REG_RELAY_CONTROL5 ) & RELAY_CONTROLBIT_PROTECTION ) {
							relay_protection_timer[ 5 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PROTECTION_TIME7 );
						}
						...
						OUT8 = 0;			// Start out at a known state
						rv = 2;
						bOn = FALSE;
						relay_pulse_flags |= 0x80;	// Enable pulse output	
						
						relay_pulse_timer[ 5 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME7 );
					}
					break;
				// Relay 8 Control
				case REG_RELAY_STATE8:
...
void doActionOn( unsigned char dmflags, unsigned char arg, unsigned char relayoffset )
{
	...
	for ( i=0; i<8; i++ ) {
		...
		if( val & RELAY_CONTROLBIT_ONEVENT ) {
			SendRelayInformationEvent( offset + 1, VSCP_CLASS1_INFORMATION, VSCP_TYPE_INFORMATION_ON );			
		}
...
void doActionOff( unsigned char dmflags, unsigned char arg, unsigned char relayoffset )
{
	...
	for ( i=0; i<8; i++ ) {
		...
		if( val & RELAY_CONTROLBIT_OFFEVENT ) {
			SendRelayInformationEvent( offset + 1, VSCP_CLASS1_INFORMATION, VSCP_TYPE_INFORMATION_OFF );			
		}
	...
	for ( i=0; i<8; i++ ) {
		...
		switch ( i ) {
			...
			case 6:
				OUT7 = 0;			// Start out at a known state
				relay_pulse_flags |= 0x40;	// Enable pulse output	
				relay_pulse_timer[ 6 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME6 );
				break;				
									
			case 7:
				OUT8 = 0;			// Start out at a known state
				relay_pulse_flags |= 0x80;	// Enable pulse output	
				relay_pulse_timer[ 7 ] = readEEPROM( VSCP_EEPROM_END + REG_RELAY_PULSE_TIME7 );
				break;				
		}
	}	
}
