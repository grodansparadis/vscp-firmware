 ...
 * Description
 *
 * RTL8019AS Receive and Transmit buffers
 * ======================================
 ...
 * DMA access (MACGetRxbuf and MACRxbufGetArray functions). One of two RTL8019AS commands can be
 * used to retrieve received packets from the RX buffer:
 * - Remote read command. The user has to set the "Remote Start Registers" and the lenght. The
 *   User also has to remove the packet from the RX buffer once read.
 * - Send Packet command. The Remote SMA registers are automatically configured to read the next
 ...
 *
 * MAC Receive Ring Buffer
 * =======================
 * The MAC makes all received packets available to the user via the MACGetRxbuf() and
 * MACRxbufGetArray() functions. These functions will read the given amount of bytes from the
 * current RX Buffer. The current receive buffer is the one we are currently reading from. The
 * MACRxbufDiscard() function will free the current receive buffer.
 *
 * Timimg:
 * =======
...
BYTE MACCurrRxbuf;   // First page of current MAC Receive Ring Buffer
WORD_VAL DMAAddr;       // Current DMA address. This variable keeps track of the Remote DMA RSAR0 and
...
#if (DEBUG_MAC > LOG_OFF)
static ROM char DebugMsgSendHdr[] = "\rM:Test";
...
void    MACInit(void)
{
    ...
    //Disable RTL8019AS interrupts. All INT lines go tri-state, and PIC pin RB0 is available to user
    NICPut(CMDR, 0xe0);         //Select page 3
    NICPut(RTL9346CR, 0xc0);    //Config register write enable
    NICPut(RTLCONF1, 0);        //Interrupt disable - will caust INT0 to be high impedance.
...
//#define WAIT_FOR_IOCHRDY() {do {NIC_IOCHRDY_TRIS=0; NIC_IOCHRDY=1; NIC_IOCHRDY_TRIS=1;} while(NIC_IOCHRDY == 0);}      //Wait for IOCHRDY pin to go high
//#define WAIT_FOR_IOCHRDY() {do {NIC_IOCHRDY_TRIS=0; NIC_IOCHRDY_TRIS=1;} while(NIC_IOCHRDY == 0);}      //Wait for IOCHRDY pin to go high
//#define WAIT_FOR_IOCHRDY() {Nop(); Nop(); Nop(); Nop(); Nop();}     // TEST TEST
...
/*********************************************************************
 * Function:        WORD    MACRxbufGet(BYTE *val, WORD len)
 *
 ...
 *
 * Input:           - len: Length of array to be read
 ...
 * Side Effects:    None
 *
 * Note:            Reads a single byte via Remote DMA from the current MAC Receive buffer.
 ...
 *                  See PreConditions above for more info.
 ********************************************************************/
...
/*********************************************************************
 * Function:        WORD    MACRxbufGetArray(BYTE *val, WORD len)
 *
 * PreCondition:    Remote DMA address has to be set up prior to calling this function.
 *                  The Remote DMA registers are NOT configured by this function,
 *                  and simply continue reading from the current "Remote DMA Address". A function
 ...
 * Side Effects:    None
 *
 * Note:            Reads the given amount of bytes via Remote DMA from the current MAC Receive buffer.
...
WORD    MACRxbufGetArray(BYTE *val, WORD len)
{
    ...
    while( len-- > 0 )
    {
        ...
        *val++ = NIC_DATA_IO;
        ...
        // HAVE TO TEST IS THESE ERRORS ARE ALSO PRESENT IN THIS READ MODE!!
        /*
        //Last byte of MAC RX Buffer is read wronge by RTL8019AS - Address has to be set again!
        if ((DMAAddr.v[0] == 0xff) && (DMAAddr.v[1] == (RXSTOP-1))) {
            //Set NIC Remote DMA read again - this fixes the RTL8019AS bug!!!!
    ...
    if (DMAAddr.Val != MACGetNICAddr()) {
        debugPutRomString((ROM char*)"\rM:RxbufGetArr Adr Err ");
    }
    #endif
...
/*********************************************************************
 * Function:        BOOL MACRxbufGetHdr(MAC_ADDR *remote, BYTE* type)
 *
 * PreCondition:    
 *
 * Input:           None
 ...
 * Side Effects:    None
 *
 * Note:            Check if the MAC Receive Buffer has any received packets.
 *                  Reads the following data from the next available packet in the RX buffer:
...
BOOL MACRxbufGetHdr(MAC_ADDR *remote, BYTE* type)
{
    ...
    *type = MAC_UNKNOWN;
    ...
    if ( NICGet(ISR) & 0x10 )
    {
        ...
        #if (DEBUG_MAC >= LOG_ERROR)
        debugPutRomString((ROM char*)"\rM:OR Err!");
        #endif
    ...
    if ( NICWritePtr != NICReadPtr )
    {
        ...
        #if (DEBUG_MAC >= LOG_ERROR)
        debugPutRomString((ROM char*)"\rM:Inval Pct Err!");
        #endif
 ...
 *                    - 6 Bytes = Desitination MAC address
 ...
 * Side Effects:    None
 *
 * Note:            This function writes the MAC header (Ethernet header) part of a packet that
 ...
 *                  NIC to transmit transmit buffer.
 *
 ********************************************************************/
...
void NICReset(void)
{
    ...
    debugPutRomString((ROM char*)"\rM:Reset!");
    #endif
...
/**
 * Set the current Remote DMA address. 
 */ 
void MACSetNICAddr(WORD addr)
{
    NICSetAddr(addr);   // TEST TEST
}
 ...
 * PreCondition:    
 *
 * Input:           - offset: Location (with respect to first byte following Ethernet header)
 *                    where next access is to occur.
 *
 * Side Effects:    None
 *
 * Note:            This function sets the access location for the active receive buffer,
 ...
 * PreCondition:    
 *
 * Input:           - buffer: A transmit buffer where this access offset be applied
 *                  - offset: Location (with respect to beginning of buffer) where next
 *                    access is to occur
 *
 * Side Effects:    None
 *
 * Note:            This function sets the access location for a given transmit buffer, and
 *                  makes that transmit buffer active. Users must make sure that the supplied
