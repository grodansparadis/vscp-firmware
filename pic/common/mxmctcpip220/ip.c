...
#if (DEBUG_IP > LOG_OFF)
static ROM char DebugMsgIpHdrId[] = "\rI:ID ";
 ...
 *                  len         - Length of IP data. For example, If TCP is contained in this IP
 ...
 * Side Effects:    None
 *
 * Note:            Only one IP message can be received.  Caller may not transmit and receive
                 ...
                 WORD *len)
{
    ...
    #if (DEBUG_IP >= LOG_INFO)
        debugPutRomString(DebugMsgIpHdrId);
        //Write HEX WORD value of tmp
        tmp = (BYTE)header.Identification;
        debugPutByteHex(tmp);
        tmp = (BYTE)(header.Identification >> 8);
        debugPutByteHex(tmp);
    #endif
...
/*********************************************************************
 * Function:        WORD IPPutHeader(   IP_ADDR *remote,
 *                                      BYTE    protocol,
 *                                      WORD    len)
 *
 ...
 *                  protocol    - Protocol of data to follow, for example ICMP, TCP....
 *                  len         - Total length of IP data bytes to follow, excluding IP header. This
 ...
 * Side Effects:    None
 *
 * Note:            Only one IP message can be transmitted at any time.
 *                  Caller may not transmit and receive a message at the same time.
 *
 ********************************************************************/
