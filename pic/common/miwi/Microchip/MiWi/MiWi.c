...
//TODO: make sure that everywhere I_AM_COORDINATOR_CAPABLE is defined we also verify that the current role of the device is a COORDINATOR as well
BYTE role;
#endif
...
void MiWiTasks(void)
{
    ...
    {         
        ...
        if(RxBuffer[0]<RX_BUFFER_SIZE)
        {
            ...
            PHYSetShortRAMAddr(WRITE_RXMCR,0b00100000);
    ...
    if(MiWiStatus.bits.RX_BUFFERED == 1)
    {
        ...
        addrMode = RxBuffer[2] & 0b11001100;
        switch(addrMode)
        {
        ...
        switch(macFrameControl.bits.frameType)
        {
            ...
            case FRAME_TYPE_DATA:
                //ConsolePutROMString((ROM char*)"Data packet\r\n");
                //The guts of MiWi ...
                {
                    WORD_VAL destPANID;
...
ThisPacketIsForMe:
							...
							if( RxBuffer[i+1] & 0x01 ) // security on
							{
								...
								{
									//ConsolePutROMString((ROM char *)"Decryption error");
									goto DiscardPacketAndFinish;
								}
							}
						#endif
                                ...
                                {                                
                                    ...
                                    case EUI_ADDRESS_SEARCH_REQUEST:
                                    {
                                        ...
                                        if(ItsMe == FALSE)
                                        {
                                            ...
                                            if(handle!=0xFF)
                                            {
                                                ...
                                                if(networkTable[handle].status.bits.shortAddressValid)
                                                {
                                            ...
                                            if(openSocketInfo.status.bits.type == P2P_SOCKET)
                                            {
                                                ...
                                                }
                                                {
                                                    ...
                                                    if(handle != 0xFF)
                                                    {
                                                        ...
                                                        //TODO - shouldn't be a command packet if it is not an association response frame
                                                        ...
                                                        if(previousEntry == FALSE)
                                                        {
                                                            ...
                                                            networkTable[handle].status.Val = 0b00110111;
                                                            for(i=0;i<8;i++)
                                                            {
                                                ...
                                                if(openSocketInfo.status.bits.type == CLUSTER_SOCKET)
                                                {
                                                    ...
                                                    {                                                
                                                        ...
                                                        if(openSocketInfo.socketHandle != 0xFF)
                                                        {
                                                            ...
                                                            tempNodeStatus.Val = networkTable[openSocketInfo.socketHandle].status.Val;
                                                        }
                                                        else
                                                        {
                                                            //otherwise create it
                                    ...
                                    case OPEN_CLUSTER_SOCKET_RESPONSE:
                                        //ConsolePutROMString((ROM char*)"found a response to my request\r\n");
                                        {
                                            ...
                                            {
                                                tempNodeStatus.Val |= 0b01000000;   //set the RxOnWhenIdle bit
                                            }
                                            ...
                                            PrintChar(openSocketInfo.socketHandle);
            ...
            case FRAME_TYPE_BEACON:
                {
                    ...
                    if(MiWiStateMachine.searchingForNetwork)
                    {
                        ...
                        if(RxBuffer[i++] == 0xFF)
                        {
                            ...
                            networkTable[entry].info.networkInfo.sampleRSSI = RxBuffer[i+8];
                            //mark this as network and not as a neighbor
                            networkTable[entry].status.bits.NeighborOrNetwork = 0;
                            //mark this as a valid entry 
                            networkTable[entry].status.bits.isValid = 1;
                        }
                    }
                    //TODO: if it is the same pan that we are a member of then rip out the routing info
                }
                break;
                ...
                switch(RxBuffer[i])
                {
                    ...
                    case MAC_COMMAND_ASSOCIATION_RESPONSE:
                        ...
                        else
                        {
                            ...
                            //TODO: verify that this is all we need to do in order to deselect this as my network
                            myParent = 0xFF;
                            myPANID.Val = 0xFFFF;
                    ...
                    case MAC_COMMAND_ORPHAN_NOTIFICATION:
                        {
                            ...
                            if(handle!=0xFF)
                            {
                                ...
                                if(networkTable[handle].status.bits.isFamily == 1)
                                {
                    ...
                    case MAC_COMMAND_BEACON_REQUEST:
                        ...
                        if(MiWiStateMachine.memberOfNetwork)
                        {
                            ...
                            if(role != ROLE_FFD_END_DEVICE)
                            {
                                ...
                                if(role == ROLE_PAN_COORDINATOR)
                                {
                                    TxBuffer[8] |= 0b01000000;
                                }
                                if(findNextNetworkEntry() != 0xFF)
                                {
                                    TxBuffer[8] |= 0b10000000;
                                }
                                TxBuffer[9]=0x00;   //GTS
...
BYTE SendReportByLongAddress(BYTE *pLongAddress)
{
    ...
    #else
    {
        ...
        toReturn = MiWiSeqNum++;
    #endif
        //TODO: send to them directly (not in table but I have their long address)           
...
BYTE SendReportByHandle(BYTE handle, BOOL forwardPacket)
{
    ...
    if(forwardPacket == FALSE)
    {
        ...
        if((TxData + 11) < TX_BUFFER_SIZE)
        {
            ...
            if(networkTable[handle].status.bits.P2PConnection && (networkTable[handle].status.bits.shortAddressValid == 0))
            {
                //I don't have direct comm with this person so send it to my parent
    ...
    if(networkTable[handle].status.bits.isValid)
    {
        #if defined(I_AM_COORDINATOR_CAPABLE)
            if(networkTable[handle].status.bits.shortAddressValid)
            {
                ...
                if(networkTable[handle].ShortAddress.v[0] == 0x00)
                {
                    ...
                    if(knownCoordinators & (1<<(networkTable[handle].ShortAddress.v[1]&0x03)))
                    {
                        ...
                        networkTable[handle].status.bits.directConnection = 1;
                    }
                }
            }
        #endif
        
        if(networkTable[handle].status.bits.directConnection)
        {
            if(networkTable[handle].status.bits.P2PConnection)
            {               
                TxBuffer[TxHeader--] = myLongAddress[7];  //MSB source long address 
                TxBuffer[TxHeader--] = myLongAddress[6];  //source long address 
                TxBuffer[TxHeader--] = myLongAddress[5];  //source long address 
            ...
            {
                if(networkTable[handle].status.bits.RXOnWhenIdle)
                {
                   if(networkTable[handle].status.bits.shortAddressValid)
                   {                 
    ...
    {
        //ConsolePutROMString((ROM char*)"Trying to transmit to invalid handle.\r\n");
        return 0xFF;
    }
...
void SendIndirectPacket(BYTE handle,BYTE dataRequestType)
{
    ...
    while(1)
    {
        ...
        if(indirectBuffer[i+1] == handle)
        {
            ...
            if((dataRequestType == DATA_REQUEST_ASSOCIATION_RESPONSE) || (networkTable[handle].status.bits.P2PConnection == 1))
            {
                TxBuffer[TxHeader--] = myLongAddress[7];  //MSB source long address 
                TxBuffer[TxHeader--] = myLongAddress[6];  //source long address 
                TxBuffer[TxHeader--] = myLongAddress[5];  //source long address 
            ...
            {
                ConsolePutROMString((ROM char*)"Short address data request\r\n");
                ...
                TxBuffer[TxHeader--] = 0x88;  //frame control -  both long addresses
                
                //TODO - shouldn't be a command packet if it is not an association response frame
                TxBuffer[TxHeader--] = 0x61;  //frame control -  command packet

            }
		#ifdef SUPPORT_SECURITY
			if( (TxBuffer[TxHeader+1] & 0b00000111) == 0x01 )
...
void DiscardPacket(void)
{
    ...
    //re-enable the ACKS
    RFIE = 0;
    PHYSetShortRAMAddr(WRITE_RXMCR,0b00000000);
...
BYTE findNextNetworkEntry(void)
{
    ...
    {
        if(networkTable[i].status.bits.isValid == 0)
        {
            return i;
        }
    }
...
void DumpNetworkTable(void)
{
    ...
    for(i=0;i<NETWORK_TABLE_SIZE;i++)
    {
        if(networkTable[i].status.bits.isValid)
        {
            if(networkTable[i].status.bits.NeighborOrNetwork == NEIGHBOR)
            {
                PrintChar(i);
                ConsolePut(' ');
                ConsolePut(' ');
                ...
                if(networkTable[i].status.bits.RXOnWhenIdle == 1)
                {
                    ConsolePut('Y');
                }
                else
                ...
                if(networkTable[i].status.bits.directConnection == 1)
                {
                    ConsolePut('Y');
                }
                else
                ...
                if(networkTable[i].status.bits.P2PConnection == 1)
                {
                    ConsolePut('Y');
                }
                else
                ...
                if(networkTable[i].status.bits.shortAddressValid)
                {
                    //this is a neighbor
                    PrintChar(networkTable[i].PANID.v[1]);
                    PrintChar(networkTable[i].PANID.v[0]);
                ...
                if(networkTable[i].status.bits.longAddressValid)
                {
...
void ClearNetworkTable(BYTE options)
{
    ...
    for(i=0;i<NETWORK_TABLE_SIZE;i++)
    {
        ...
        {
            if(networkTable[i].status.bits.directConnection == 0)
            {
                networkTable[i].status.bits.isValid = 0;
            }
        }
        if(clearOptions.bits.b1)
        {
            if(networkTable[i].status.bits.longAddressValid == 0)
            {
                networkTable[i].status.bits.isValid = 0;
            }
        }
        ...
        {
            if(networkTable[i].status.bits.shortAddressValid == 0)
            {
                if(networkTable[i].status.bits.P2PConnection == 0)
                {
                    networkTable[i].status.bits.isValid = 0;
                }
            }
...
void JoinNetwork(BYTE handle)
{
    ...
    if(networkTable[handle].status.bits.isValid == 1)
    {
        ...
        TxBuffer[0]=0b00100011;   //frame control - command
        TxBuffer[1]=0xC8;   //addressing mode - source only
        TxBuffer[2]=IEEESeqNum++;   //seq number 
        ...
        TxBuffer[18]=CAP_INFO;  //cap info of requesting device //TODO: make this dynamic
...
BYTE SearchForShortAddress(BYTE entryType)
{
    ...
    for(i=0;i<NETWORK_TABLE_SIZE;i++)
    {
        if(networkTable[i].status.bits.isValid)
        {
            if(networkTable[i].status.bits.NeighborOrNetwork == entryType)
            {
...
BYTE AddNodeToNetworkTable(void)
{
    ...
    { 
        ...
        networkTable[handle].status.Val = tempNodeStatus.Val;
        if(tempNodeStatus.bits.longAddressValid)
        {
 ...
 void MiWiInit(void)
{
    ...
    //TODO: may want to reload a EEPROM version if available
    //TODO: make sure all variables are initialized correctly
    #if !defined(P2P_ONLY)
        ClearNetworkTable(CLEAR_ALL_ENTRIES);
    #endif
    
    #if defined(I_AM_COORDINATOR_CAPABLE)
    //TODO: do we always want to clear the routing table?  what about after a power failure? EEPROM?
    for(i=0;i<8;i++)
    {
        RoutingTable[i] = 0;
    }
...
void MRF24J40Wake(void)
{
    ...
    while(1)
    {
        ...
        if(currentDifference.Val > 250)
        {
            break;
        }
    
        results = PHYGetLongRAMAddr(RFSTATE);
        if((results & 0b11100000) == 0b10100000)
        {
            break;
        }
    }
...
CipherOperationStart:
	...
	// Todo: working with interrupt handler to solve the problem in case
...
BOOL DataDecrypt(IOPUT BYTE *Input, IOPUT BYTE *DataLen, INPUT BYTE *Header, INPUT BYTE HeaderLen)
{
	...
	{
		// ToDo: if keys can be requested/transferred in application layer
	...
	if( (i != 0xFF) && networkTable[i].status.bits.isFamily )
	{
