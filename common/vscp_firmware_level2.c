...
void vscp_readRegister( vscpEvent *pEvent, vscpEvent *pOutEvent )
{
	...
	for ( i=0; i<cnt; i++ ) {
		...
		if ( ( idx + i ) > VSCP_LEVEL2_COMMON_REGISTER_START ) {
			// Common register
			pOutEvent->data[ 8 + i ] = vscp_readStdReg( idx - VSCP_LEVEL2_COMMON_REGISTER_START + 0x80 + i );
		}
		else {
			// User register
...
void vscp_writeRegister( vscpEvent *pEvent, vscpEvent *pOutEvent )
{
	...
	for ( i=0; i<cnt; i++ ) {
		...
		if ( ( idx + i ) > VSCP_LEVEL2_COMMON_REGISTER_START ) {
			// Common register
			pOutEvent->data[ 8 + i ] = vscp_writeStdReg( ( idx & 0xff ) + i, pEvent->data[ 24 + i ] );
		}
		else {
			// User register
		    pOutEvent->data[ 8 + i ] = vscp_writeAppReg( idx + i, pEvent->data[ 24 + i ] );
		}
...
void fillGUID( PVSCPEVENT pevent )
{
	uint8_t i;
