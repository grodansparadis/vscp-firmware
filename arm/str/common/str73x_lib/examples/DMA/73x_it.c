...
{
  /* clear DMA3 stream3 transfer end interrupt(SI3) flag */
  DMA_FlagClear(DMA3, DMA_FLAG_INT3);

  /* disable DMA access on AHB bus */
  DMA_AHBArbitrationConfig(DMA_Priority_Low);
  
  /* check if the data transfered by the DMA is correct ----------------------*/
  TransferStatus = Buffercmp(DST_Buffer, SRC_Buffer, NbrDataToTransfer);
  /* TransferStatus = PASS, if the data transfered by the DMA is correct */
  /* TransferStatus = FAIL, if the data transfered by the DMA is corrupted */	
}
