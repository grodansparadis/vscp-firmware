...
* Input 1        : UARTx (x can be 1 or 2) the desired UART
* Output         : None
* Return         : None
*******************************************************************************/
...
* Return         : None
*******************************************************************************/
void UART_BaudRateConfig(UART_TypeDef *UARTx, vu32 BaudRate)
{
  UARTx->BRR =(vu16)(RCCU_GetFrequency(RCCU_AAPBClock)/(16*BaudRate));
}
...
* Return         : None
*******************************************************************************/
void UART_Config(UART_TypeDef *UARTx, vu32 BaudRate, UARTParity_TypeDef Parity,
...
* Return         : None
*******************************************************************************/
void UART_ITConfig(UART_TypeDef *UARTx, vu16 UART_Flag, FunctionalState NewState)
{
...
* Return         : None
*******************************************************************************/
void UART_ByteSend(UART_TypeDef *UARTx, vu8 *Data)
{
...
* Return         : None
*******************************************************************************/
void UART_9BitByteSend(UART_TypeDef *UARTx, vu16 *Data)
{
  if(UARTx->CR & (0x0001<<UART_FIFOEnableBit))// if FIFO ENABLED
    while((UARTx->SR & UART_TxFull)); // while the UART_TxFIFO contain 16 characters.
  else                  // if FIFO DISABLED
...
* Return         : None
*******************************************************************************/
void UART_DataSend(UART_TypeDef *UARTx, vu8 *Data, vu8 DataLength)
{
  vu8 *DataToSend;
  DataToSend=Data;
  while(DataLength--)
  {
    UART_ByteSend(UARTx,DataToSend);
    DataToSend++;
  }
}
...
* Return         : None
*******************************************************************************/
void UART_9BitDataSend(UART_TypeDef *UARTx, vu16 *Data, vu8 DataLength)
{
  while(DataLength--)
  {
...
* Return         : None
*******************************************************************************/
void UART_StringSend(UART_TypeDef *UARTx, vu8 *String)
{
  vu8 *Data=String;
  while(*Data != '\0')
...
*******************************************************************************/
u16 UART_ByteReceive(UART_TypeDef *UARTx, vu8 *Data, vu8 TimeOut)
{
   u16 wStatus;
...
* Return         : The UARTx.SR register contents
*******************************************************************************/
u16 UART_9BitByteReceive(UART_TypeDef *UARTx, vu16 *Data, vu8 TimeOut)
{
  u16 wStatus;
  UARTx->TOR=TimeOut;// reload the Timeout counter
  while (!((wStatus=UARTx->SR) & (UART_TimeOutIdle|UART_RxHalfFull|UART_RxBufFull)));// while the UART_RxFIFO is empty and no Timeoutidle
...
* Return         : The UARTx.SR register contents
*******************************************************************************/
u16 UART_DataReceive(UART_TypeDef *UARTx, vu8 *Data, vu8 DataLength, vu8 TimeOut)
{
  u16 wStatus;
  while(DataLength--)
...
* Return         : The UARTx.SR register contents
*******************************************************************************/
u16 UART_9BitDataReceive(UART_TypeDef *UARTx, vu16 *Data, vu8 DataLength, vu8 TimeOut)
{
  u16 wStatus;
  while(DataLength--)
...
* Return         : The UARTx.SR register contents
*******************************************************************************/
u16 UART_StringReceive(UART_TypeDef *UARTx, vu8 *Data)
{
  vu8 *pSTRING=Data;
  u16 wStatus;
  do
