...
extern u16 potentiometer_ad_Value;
extern u16 termistor_ad_Value;
...
void TB2_IRQHandler (void)
{
	...
	if ( GPIO_BitRead( GPIO2, GPIO_PIN_10 ) ) {
		// Active
		vscp_initbtncnt++;
	}
	else {
	...
	if ( ( VSCP_LED_BLINK1 == vscp_initledfunc ) && ( vscp_statuscnt > 100 ) ) {
		...
		if ( GPIO_BitRead( GPIO0, GPIO_PIN_0 ) ) {
...
{
	// Get ADC value
  potentiometer_ad_Value = ADC_GetConversionValue( ADC_CHANNEL0 );
	
	// Get termistor value
  termistor_ad_Value = ADC_GetConversionValue( ADC_CHANNEL1 );
	
	// Clear EOC Interrupt pending bit 
  ADC_FlagClear( ADC_FLAG_EOC );
  
	// Clear the last Analog Watchdog0 comparison result */
  ADC_FlagClear( ADC_FLAG_AnalogWatchdog0_HighThresold | 
										ADC_FLAG_AnalogWatchdog0_LowThresold );
