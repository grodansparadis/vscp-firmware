		 	   ...
		 	   if (wBlockNum == 0 )   /* Decode the Special Command*/
                           {
                            ...
                            if ( (Load_Buffer[0] ==  CMD_GETCOMMANDS) &&(wlength == 1) )
...

                             {
                              ...
                              if  ( Pointer < 0x80000 )  /* Internal Flash */
                              {

                                Internal_FLASH_SectorErase(Pointer+0x80000);
                              }
				    ...
				    DATA = *((u32*)&Load_Buffer[i]);
                                    if ( Addr < 0x80000 )   /* Internal flash programming */
     			   	      Internal_FLASH_WordWrite(Addr+0x80000,DATA);
                                    /*else
...
DEVICE_INFO		*pInfo = &Device_Info;
	...
	else if(wBlockNum > 1)
	{
		...
		DeviceStatus[3]=0;
                ...
                if (Pointer < 0x80000)
	        Phy_Addr=(u8*)(Addr+0x80000);
                if ((Pointer >= 0x80000) && (Pointer < 0x90000)) /* Read bank1 */
	        Phy_Addr=(u8*)(Addr-0x80000);
...
u8 *DNLOAD (u16 Length)
{
	...
	/*if (((wBlockNum-2) * wTransferSize)+Pointer) >= 0x400C4000  )
        {
							
                   DeviceState = STATE_dfuERROR;
		   DeviceStatus[4]=  DeviceState;
		   DeviceStatus[0] = STATUS_ERRADDRESS;
		   DeviceStatus[1]=0;
                   DeviceStatus[2]=0;
                   DeviceStatus[3]=0;
	           return (NULL);
              ...
              if (wlength != 0)
			              ...
			              DeviceStatus[4]=DeviceState;
                                  ...
                                  if ((wBlockNum == 0) && (Load_Buffer[0] == CMD_ERASE) )
                                    {
                                        if  (Pointer <= 0x80000) /* 64K sectors */
                                             {
