	...
	: CDialog(CTestDriverDlg::IDD, pParent)
{
	...
	m_szOption2 = _T("");
	m_szOption1 = _T("");
	m_strConfiguration = _T("4;1;115200;500");
	m_EditBurstCount = _T("");
	m_sentFrames = _T("");
	m_receivedFrames = _T("");
	m_bTimeStamp = FALSE;
...
void CTestDriverDlg::OnButtonSendMsg() 
{
	...
	UpdateData( true );
	getDataValues( &msg );
	if ( CANAL_ERROR_SUCCESS !=  CanalSend( m_drvHandle, &msg ) ) {
...
void CTestDriverDlg::OnButtonSendBurts() 
{
	...
	for ( long i=0; i<cnt; i++ ) {
		...
		if ( CANAL_ERROR_SUCCESS != ( rv = CanalSend( m_drvHandle, &msg ) ) ) {
...
void CTestDriverDlg::OnButtonBigBurst() 
{
	...
	int rv;
	for ( long i=0; i<cnt; i++ ) {
		while ( CANAL_ERROR_FIFO_FULL == 
				( rv = CanalSend( m_drvHandle, &msg ) ) ) {
			Sleep( 1 );
		}
	}
...
void CTestDriverDlg::OnButtonVirtualTest() 
{
	...
	unsigned char a,b,c,d;
	unsigned long flags = 0x00000000;
	HANDLE hTreadTransmit;
	canalMsg msg;

	// Must be open
	if ( !m_bOpen ) {
		AfxMessageBox("Must be open!");
		return;
	}
	...
	if ( 0 == strlen( m_strConfiguration ) ) {
		AfxMessageBox("A serial number is needed for the virtual test!");
		return;
	}
...
/*
	flags = ( ( m_bTimeStamp ) ? CANUSB_FLAG_TIMESTAMP : 0 ) | 
			( ( m_bQueueReplace ) ? CANUSB_FLAG_QUEUE_REPLACE : 0 ) |
			( ( m_bBlockingMode ) ? CANUSB_FLAG_BLOCK : 0 ) |
			( ( m_bSlowMode ) ? CANUSB_FLAG_SLOW : 0 ) |
			( ( m_bNoLocalSend ) ? CANUSB_FLAG_NO_LOCAL_SEND : 0 );
*/
	...
	// Open channel
	if ( 0 >= ( h = CanalOpen( m_strConfiguration,
									flags ) ) ) {
		// Failed to open driver
		AfxMessageBox("Failed to open CANUSB interface." );
		return;
	...
	// Send some message
	for ( i=0; i<10; i++ ) {
		if ( CANAL_ERROR_SUCCESS !=  CanalSend( h, &msg ) ) {
			AfxMessageBox("Failed to send message");
		}
	}
	...
	txObj.bRun = true;
	*txObj.log = 0;
	//txObj.flags = CANUSB_FLAG_TIMESTAMP;
	txObj.flags = 0;
	strcpy( txObj.szDevice, m_strConfiguration );

	// Start write thread 
	DWORD ThreadId;
	if ( NULL == ( hTreadTransmit = 
					CreateThread(	NULL,
									0,
	...
	if ( CANAL_ERROR_SUCCESS != CanalClose( h ) ) {
		// Failed to close driver
		AfxMessageBox("Failed to close CANUSB interface." );
		return;
	}
...
void workThreadTransmit( void *pObject )
{
	...
	DWORD errorCode = 0;
	int drvhandle, rv;
	txstruct *ptxObj = (txstruct *)pObject;
	canalMsg msg;

	if ( 0 >= ( drvhandle = CanalOpen( ptxObj->szDevice,
											ptxObj->flags ) ) ) {
		ExitThread( -1 );	 
	}
	...
	for ( j=0; j<8; j++ ) {
		...
		for ( i=0; i<100; i++ ) {
			...
			if ( CANAL_ERROR_SUCCESS !=  ( rv = CanalSend( drvhandle, &msg ) ) ) {
				CanalClose( drvhandle );
				ExitThread( -3 );
			}
	...
	//if ( CANAL_ERROR_SUCCESS != ( rv = canusb_Flush( drvhandle, 0 ) ) ) {
	//	CanalClose( drvhandle );
	//	ExitThread( -4 );
	//}

	if ( CANAL_ERROR_SUCCESS != ( rv = CanalClose( drvhandle ) ) ) {
		ExitThread( -2 );
	}
...
/*
	...
	// Clear the Combo
	m_ctrlComboAdapters.Clear();
	cnt = m_ctrlComboAdapters.GetCount();
	while (cnt) {
		m_ctrlComboAdapters.DeleteString( 0 );
		cnt = m_ctrlComboAdapters.GetCount();
	}

	// Fill the box with serial numbers if there are any
	// adapters avaiable

	if ( ( cnt = canusb_getFirstAdapter( szAdapter, sizeof( szAdapter ) ) ) > 0 ) {
		str = szAdapter;
		m_ctrlComboAdapters.AddString( str );
		for ( i=1; i<cnt; i++ ) {
			canusb_getNextAdapter( szAdapter, sizeof( szAdapter ) );
			str = szAdapter;
			m_ctrlComboAdapters.AddString( str );
		}

		// Select the first
		m_ctrlComboAdapters.SetCurSel( 0 );
	}
*/
}
...
/*
	...
	Frame.channel = 0;
	Frame.flags = 7;		// 0x0f bytecount -
	Frame.data[0] = 0x0F | VSCP_SERIAL_CAN_DATA0_EXTENDED;	
	Frame.data[1] = 0xFF;
	Frame.data[2] = 0xFF;
	Frame.data[3] = 0xFF; //FFFFFFF
	Frame.data[4] = 0x11;
	Frame.data[5] = 0x22;
	Frame.data[6] = 0x33;
	Frame.opcode = VSCP_SERIAL_OPCODE_LEVEL1_EVENT;
	Frame.vscpclass = 0;
	Frame.vscptype = 20;
	
	for ( i=0; i<10; i++ ) {
		Frame.seqnumber = i % 256;
		vs_sendFrame( &Frame );
	}

	while ( 1 ) {
		//c = gcom.readChar( &cnt );
		cnt = dev->Read( buf, sizeof( buf ) );
		if ( cnt ) {
			for ( i=0; i<cnt; i++ ) {
				if ( vs_feed( buf[ i ] ) ) {
					AfxMessageBox("Frame received");
				}
			}
		}
		else {
