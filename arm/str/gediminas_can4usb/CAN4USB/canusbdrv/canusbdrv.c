// canusbdrv.cpp : Defines the entry point for the DLL application.
//
...
void InitInstance( DriverObj *pdrvObj )
{
	...
	for ( i = 0; i<CANUSB_MAX_CANNELS; i++ ) {
...
ChannelObj *getDriverObject( DriverObj *pdrvObj, long h )
{
	...
	if ( idx >= CANUSB_MAX_CANNELS ) return NULL;
...
void removeDriverObject( DriverObj *pdrvObj, long h )
{
	...
	// Check if valid handle
	if ( idx < 0 ) return;
	if ( idx >= CANUSB_MAX_CANNELS ) return;
...
long WINAPI CanalOpen( const char *pDevice, unsigned long flags )
{
	...
	//		Acceptance Filter
	acceptance_filter = CANUSB_ACCEPTANCE_FILTER_ALL;	// Set default
	p = strtok( NULL, ";" );
	if ( NULL != p ) {
	...
	//		Acceptance Mask
	acceptance_mask = CANUSB_ACCEPTANCE_MASK_ALL;		// Set default
	p = strtok( NULL, ";" );
	if ( NULL != p ) {
					...
					uint32_t flags ) 
{
  int i;
	...
	pchannelObj->m_comMutex = CreateMutex( NULL, TRUE, NULL );			// CANUSB_DLL_COMCHANNEL_MUTEX
	pchannelObj->m_transmitMutex = CreateMutex( NULL, TRUE, NULL ); // CANUSB_DLL_TRANSMIT_MUTEX
	...
	pchannelObj->m_receiveCharEvent = CreateEvent( NULL,
													FALSE,
													FALSE,
													NULL );			// CANUSB_DLL_RECEIVE_CHAR_EVENT
	...
	pchannelObj->m_transmitFrameEvent = CreateEvent( NULL,
														FALSE,
														FALSE,
														NULL );		// CANUSB_DLL_TRANSMIT_FRAME_EVENT
	...
	pchannelObj->m_AckNackEvent = CreateEvent( NULL,
												FALSE,
												FALSE,
												NULL );				// CANUSB_DLL_ACKNACK_EVENT

	// transmit block
	pchannelObj->m_transmitRemoveEvent = CreateEvent( NULL,
														TRUE,
														TRUE,
														NULL );		// CANUSB_DLL_TRANSMIT_REMOVE_EVENT
...
int canreadMsg( ChannelObj *pchannelObj, CANHANDLE channel, canalMsg *pMsg, BOOL bBlocking )
{
	...
	// Should we block
	if ( bBlocking /*pchannelObj->m_flags & CANUSB_FLAG_BLOCK*/ ) {
...
int canwriteMsg( ChannelObj *pchannelObj, CANHANDLE channel, canalMsg *pMsg, BOOL bBlocking  )
{
	...
	if ( !( pchannelObj->m_flags & CANUSB_FLAG_NO_LOCAL_SEND ) ) {
		...
		while ( NULL != pvirtualNode  ) {
			...
			if ( ( pchannelObj->m_flags & CANUSB_FLAG_QUEUE_REPLACE ) && 
						...
						switch ( serialFrame.opcode ) {
							...
							case VSCP_SERIAL_OPCODE_CAN_FRAME:
								...
								while ( NULL != pvirtualNode  ) {
									...
									// Noting to do if no object
									if ( NULL == ( pvirtualChannelObj = pvirtualNode->pObject ) ) {
										pvirtualNode = pvirtualNode->pNext;
										continue;
									}
									...
									// the queue if the QUEUE_REPLACE flag is set to be able to
									// add the new message at the end.
									if ( ( pobj->m_flags & CANUSB_FLAG_QUEUE_REPLACE ) && 
										( dll_getNodeCount( &pvirtualChannelObj->m_rxQueue ) >= CANBUS_RXFIFO_SIZE ) ) {
