...
/* Private variables ---------------------------------------------------------*/
  ...
  extern u8 RxBuffer0[];
  extern u8 RxBuffer2[];
  extern u8 TxCounter0;
  extern u8 TxCounter2;
  extern u8 RxCounter0; 
  extern u8 RxCounter2;
  extern u8 NbrOfDataToTransfer0;
  extern u8 NbrOfDataToTransfer2;
  extern u8 NbrOfDataToRead0;
...
void UART2_IRQHandler(void)
{
  ...
  if(UART_GetITStatus(UART2, UART_IT_Receive) != RESET)
  {
    /* Read 2 bytes from the receive FIFO */
    for(m = 0; (m < 2) && (RxCounter2 < NbrOfDataToRead2); m++)
    {
      RxBuffer2[RxCounter2++] = UART_ReceiveData(UART2);      
    }

    if(RxCounter2 != NbrOfDataToRead2)
    {
      /* Clear the UART2 Receive interrupt */
  ...
  if(UART_GetITStatus(UART2, UART_IT_Transmit) != RESET)
  {   
    /* Write 8 bytes to the transmit FIFO */
    for(l = 0; (l < 8) && (TxCounter2 < NbrOfDataToTransfer2); l++)
    {
    ...
    {
      /* Clear the UART2 transmit interrupt */
...
void UART0_IRQHandler(void)
{
  ...
  {
    /* Read 2 bytes from the receive FIFO */
    for(j = 0; (j < 2) && (RxCounter0 < NbrOfDataToRead0); j++)
    {
      RxBuffer0[RxCounter0++] = UART_ReceiveData(UART0);      
    }

    if(RxCounter0 != NbrOfDataToRead0)
    {
    ...
    {
      /* Disable the UART0 Receive interrupt */
  ...
  {   
    /* Write 8 bytes to the transmit FIFO */
    for(i = 0; (i < 8) && (TxCounter0 < NbrOfDataToTransfer0); i++)
    {
      UART_SendData(UART0, TxBuffer0[TxCounter0++]);                    
    }

    if(TxCounter0 != NbrOfDataToTransfer0)
    {
