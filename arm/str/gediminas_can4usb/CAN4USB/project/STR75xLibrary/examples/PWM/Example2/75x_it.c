...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
u16 Pulse_Table[11] = {0x000F, 0x001F, 0x003F, 0x007F, 0x00FF, 
                       0x03FF, 0x07FF, 0x0FFF, 0x3FFF, 0x7FFF, 0xBFFF};
u16 i = 0;
u16 Pulse_Direction1 = 0x1, Pulse_Direction2 = 0x2;
u16 Pulse_Direction = 0x1;
...
{
  if(Pulse_Direction == Pulse_Direction1)
  {
    PWM_SetPulse(PWM_Channel_ALL, Pulse_Table[i++]);

    if(i == 10)
    {
      Pulse_Direction = Pulse_Direction2;
    } 
  }
  else 
  {
    ...
    if(i == 0)
    {
      Pulse_Direction = Pulse_Direction1;
    } 
  }
  
  PWM_ClearITPendingBit(PWM_IT_Update);
}
