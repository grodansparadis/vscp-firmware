...
esint8 file_fclose(File *file)
{
	if(fs_hasTimeSupport()){
		...
		file->DirEntry.AccessDate = time_getDate(); /* moved closer to time */
 ...
 * Description: This function read byte from file and adjusts fileptr.
...
esint16 file_getc(File* file)
{
    ...
    r = file_fread(file, file->FilePtr, sizeof(euint8), &byte) ;
    if(r == 1) {
	file->FilePtr+=r;
        return(1);
    } else {
...
/* ****************************************************************************
 * esint16 file_putc(File *file)
 * Description: This function writes byte to file and adjusts fileptr.
 * Return value: 1 on success and -1 on failure.
*/
esint16 file_putc(File* file, euint8* byte)
{
    euint32 r;
    r = file_fwrite(file, file->FilePtr, sizeof(euint8), byte);
 ...
 * Description: This function determines if EOF has been reached.
 * Return value: 1 if EOF and 0 if not.
*/
euint8 file_feof(File* file)
{
    if(file->FilePtr == file->FileSize)
      return(1);
    else
      return(0);
}
