...
static bool move_window (DWORD sector){		/* Sector number to make apperance in the FatFs->win */
...
#ifndef _FS_READONLY
static bool remove_chain (DWORD clust){			/* Cluster# to remove chain from */
	DWORD nxt;
...
static BYTE check_fs (DWORD sect){		/* Sector# to check if it is a FAT boot record or not */
...
static bool next_dir_entry (DIR *scan){			/* Pointer to directory object */
	DWORD clust;
	WORD idx;
	FATFS *fs = FatFs;
...
FRESULT f_getfree (DWORD *nclust){		/* Pointer to the double word to return number of free clusters */
...
return ((27<<25)|(3<<21)|(1<<16)|(2<<11)|(46<<5)|(13<0));
}
