...
cs8900a_write(unsigned addr, unsigned int data)
{
  ...
  IO0CLR = 0xff << 16;                            // Write low order byte to data bus
  IO0SET = data << 16;
  ...
  IO0CLR = 0xff << 16;                            // Write high order byte to data bus
  IO0SET = data >> 8 << 16;
...
cs8900a_read(unsigned addr)
{
  ...
  value = (IO0PIN >> 16) & 0xff;                  // get low order byte from data bus
  IO0SET = IOR;

  IO0SET = 1 << 4;                                // IOR high and put next address on bus

  IO0CLR = IOR;                                   // IOR-signal low
  value |= ((IO0PIN >> 8) & 0xff00);              // get high order byte from data bus
  IO0SET = IOR;                                   // IOR-signal low
...
cs8900a_init(void)
{
  ...
  // No LEDs on.
  IO0SET = LED_RED | LED_YELLOW | LED_GREEN;

  // Port 3 as output (all pins but RS232)
  IO0DIR = ~0U; // everything to output.
...
cs8900a_send(void)
{
  ...
  IO0CLR = LED_RED;  // Light RED LED when frame starting
  ...
  for (;;)
    {
      ...
      if (u -- == 0)
        {
          IO0SET = LED_RED;  // Extinguish RED LED on end of frame
          return;
        }
  ...
  for (u = 0; u < 54; u += 2)
    {
      ...
      IO0CLR = 0xff << 16;                            // Write low order byte to data bus
      IO0SET = uip_buf[u] << 16;                      // write low order byte to data bus
      ...
      IO0CLR = 0xff << 16;                            // Write low order byte to data bus
      IO0SET = uip_buf[u+1] << 16;                    // write low order byte to data bus
  ...
  for (u = 0; u < uip_len; u += 2)
    {
      ...
      IO0CLR = 0xff << 16;                        // Write low order byte to data bus
      IO0SET = uip_appdata[u] << 16;              // write low order byte to data bus
      ...
      IO0CLR = 0xff << 16;                        // Write low order byte to data bus
      IO0SET = uip_appdata[u+1] << 16;            // write low order byte to data bus
...
cs8900a_poll(void)
{
  ...
  IO0CLR = LED_GREEN;  // Light GREED LED when frame coming in.
  ...
  while (u < len)
    {
      ...
      IO0CLR = IOR;                               // IOR-signal low
      uip_buf[u] = IO0PIN >> 16;                // get high order byte from data bus
      IO0SET = IOR;                               // IOR-signal high

      IO0SET = 1 << 4;                            // put address on bus

      IO0CLR = IOR;                               // IOR-signal low
      uip_buf[u+1] = IO0PIN >> 16;                  // get high order byte from data bus
      IO0SET = IOR;                               // IOR-signal high
      u += 2;
    }

  IO0SET = LED_GREEN;  // Extinguish GREED LED when frame finished.
  return len;
}
