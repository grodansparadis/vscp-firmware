...
// fILE From Rowley Associates uip port for the Olimex LPC-E212x boards
// Changes 2006 by Ake Hedman, eurosource <akhe@eurosource.se>
// for the VSCP project. http://www.vscp.org
...
{
	IODIR0 |= 0xff << 16;          // Data port to output

	IOCLR0 = 0xf << 4;             // Put address on bus
	IOSET0 = addr << 4;
  
	IOCLR0 = 0xff << 16;                            // Write low order byte to data bus
	IOSET0 = data << 16;
	...
	IOCLR0 = 0xf << 4;
	IOSET0 = ((addr | 1) << 4);                     // And put next address on bus

	IOCLR0 = 0xff << 16;                            // Write high order byte to data bus
	IOSET0 = data >> 8 << 16;
...
unsigned cs8900a_read( unsigned addr )
{
	...
	IODIR0 &= ~(0xff << 16);                        // Data port to input

	IOCLR0 = 0xf << 4;                              // Put address on bus
	IOSET0 = addr << 4;

	IOCLR0 = IOR;                                   // IOR-signal low
	value = (IOPIN0 >> 16) & 0xff;                  // get low order byte from data bus
	IOSET0 = IOR;

	IOSET0 = 1 << 4;                                // IOR high and put next address on bus

	IOCLR0 = IOR;                                   // IOR-signal low
	value |= ((IOPIN0 >> 8) & 0xff00);              // get high order byte from data bus
	IOSET0 = IOR;                                   // IOR-signal low
  
	return value;
}

///////////////////////////////////////////////////////////////////////////////
//cs8900a_read_addr_high_first
//
// Reads a word in little-endian byte order from a specified port-address
//
...
unsigned cs8900a_read_addr_high_first( unsigned addr )
{
	...
	IOCLR0 = IOR;                                   // IOR-signal low
	value = ((IOPIN0 >> 8) & 0xff00);               // get high order byte from data bus
	IOSET0 = IOR;                                   // IOR-signal high

	IOCLR0 = 1 << 4;                                // Put low address on bus
...
void cs8900a_send(void)
{
	...
	IOCLR0 = LED_RED;  // Light RED LED when frame starting
	...
	for (;;) {
		...
		if (cs8900a_read(DATA_PORT) & READY_FOR_TX_NOW)
			break;
		if (u -- == 0) {
			IOSET0 = LED_RED;  // Extinguish RED LED on end of frame
			return;
        }
	...
	for (u = 0; u < 54; u += 2) {
		IOCLR0 = 0xf << 4;                              // Put address on bus
		IOSET0 = TX_FRAME_PORT << 4;

		IOCLR0 = 0xff << 16;                            // Write low order byte to data bus
		IOSET0 = uip_buf[u] << 16;                      // write low order byte to data bus

		IOCLR0 = IOW;                                   // Toggle IOW-signal
		IOSET0 = IOW;

		IOCLR0 = 0xf << 4;                              // Put address on bus
		IOSET0 = (TX_FRAME_PORT | 1) << 4;              // and put next address on bus

		IOCLR0 = 0xff << 16;                            // Write low order byte to data bus
		IOSET0 = uip_buf[u+1] << 16;                    // write low order byte to data bus
	...
	for (u = 0; u < uip_len; u += 2) {
		...
		IOCLR0 = 0xff << 16;                        // Write low order byte to data bus
		IOSET0 = uip_appdata[u] << 16;              // write low order byte to data bus
		...
		IOCLR0 = 0xff << 16;                        // Write low order byte to data bus
		IOSET0 = uip_appdata[u+1] << 16;            // write low order byte to data bus
...
u8_t cs8900a_poll(void)
{
	...
	IOCLR0 = LED_GREEN;  // Light GREED LED when frame coming in.
	...
	IOCLR0 = 0xf << 4;                          // put address on bus
	IOSET0 = RX_FRAME_PORT << 4; 

	// Read bytes into uip_buf
	u = 0;
	while (u < len) {
		IOCLR0 = 1 << 4;                    	// put address on bus

		IOCLR0 = IOR;                       	// IOR-signal low
		uip_buf[u] = IOPIN0 >> 16;             // get high order byte from data bus
		IOSET0 = IOR;                        	// IOR-signal high

		IOSET0 = 1 << 4;                    	// put address on bus

		IOCLR0 = IOR;                               // IOR-signal low
		uip_buf[u+1] = IOPIN0 >> 16;                  // get high order byte from data bus
		IOSET0 = IOR;                               // IOR-signal high
		u += 2;
    }

	IOSET0 = LED_GREEN;  // Extinguish GREED LED when frame finished.
	return len;
}
