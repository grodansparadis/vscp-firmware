...
{
  uint8_t guid1[16] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x10 };
  uint8_t guid2[16];
  char strguid[48];
  int rv;
...
{
  vscpEvent ev;
  const char* xml = "<event head=\"2\" obid=\"5678\" timestamp=\"99999\" class=\"25\" type=\"10\" guid=\"AA:BB:CC:DD:EE:FF:00:11:22:33:44:55:66:77:88:99\" />";
...
TEST(_vscp_firmware_helper, vscp_fwhlp_parse_xml_event_max_values)
{
  ...
  ASSERT_EQ(65535, ev.head);
  ASSERT_EQ(4294967295U, ev.obid);
  ASSERT_EQ(4294967295U, ev.timestamp);
  ASSERT_EQ(65535, ev.vscp_class);
  ASSERT_EQ(65535, ev.vscp_type);
  
  // Verify GUID is all 0xFF
  uint8_t max_guid[16];
  memset(max_guid, 0xFF, 16);
  ASSERT_EQ(0, memcmp(max_guid, ev.GUID, 16));
}
