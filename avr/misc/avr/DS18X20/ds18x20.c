...
unsigned char ds18x20_system_msg=0;
...
static void send_selected_rom(void)
{
/* If only one device is on the bus we can use OW_SKIP_ROM command which have less overhead */
...
signed int ds18x20_get_temp(void)
{
signed int     temperature=0;
signed char    bit=0;
unsigned char  t_resolution=0;

/* First we need to know the DS18B20 resolution if it exists of course */
...
/* Second we must see if the DS18X20 operates in parasite mode */
ds18x20_get_power_status();
ow_reset();
send_selected_rom();
ow_put_byte(CONVERT_T);

/* If in parasite mode then delay until conversion is over else check using the ow_get_bit() */
if(power_status == 0) 
 {
    ow_pull_hard_dq(1);
