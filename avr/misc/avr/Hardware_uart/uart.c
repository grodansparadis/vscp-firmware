...
/* Usefull macro definitions and register location finding */
...
{   
   if(*tx_data && tx_data && tx_on[uart]) /* check for TX_on, a NULL pointer or an EMPTY string */
    {
      while(tx_busy[uart]);
      tx_busy[uart]=1; data_location[uart]=LOCATION_IS_RAM;
...
{   
  if(tx_on[uart]) /* If TX_on send the string else terminate */
   {
      while(tx_busy[uart]); 
      tx_busy[uart]=1; data_location[uart]=LOCATION_IS_FLASH;
...
{   
  ...
  if(tx_on[uart]) /* If TX_on send the string else terminate */
   {
      while(tx_busy[uart]); 
      tx_busy[uart]=1; data_location[uart]=LOCATION_IS_EEPROM;
