...
#define LCD_E_DELAY()             { __asm__ volatile("nop"); __asm__ volatile("nop"); }
...
#elif LCD_IO_MODE == 6
/*######################################################################################################*/
#define lcd_toggle_e()   ({ cbi(LCD_E_PORT, LCD_E_PIN); LCD_E_DELAY(); sbi(LCD_E_PORT, LCD_E_PIN);  })
...
unsigned char lcd_data[6]={'0','0','0','0','0','0' }, position=sizeof(lcd_data), radix=10; 
        ...
        /* Position starts at array position 5, so minimum is array position 1 */
        ...
        if( dot_position<sizeof(lcd_data) && dot_position)
         {
            while( (sizeof(lcd_data)-dot_position) <= position )  { position--; }
            number_of_chars--;
         }   

        /* max number of chars is sizeof(lcd_data)=6 so min position is 6-6=0 */
        if(number_of_chars<=sizeof(lcd_data)) 
         {
            position=sizeof(lcd_data)-(number_of_chars);
         } 

        /* Display the number on the LCD screen */
        for(; position<=(sizeof(lcd_data)-1); position++)
          {
            if( position==sizeof(lcd_data)-dot_position ) { lcd_putc(LCD_DECIMAL_POINT); } 
