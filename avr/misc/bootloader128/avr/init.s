...
BOOT_PURE_RESET:       ; Code entered when the device is reseted

      CLR R25		   ;  R25:R24 serves as first parameter to main()
      CLR R24		   ;  set the parameter to 0 (reset)
      RJMP INIT

BOOT_APP_CALL:         ;  Code entered after 'boot' request from the application 
...
INIT:
      ...
      LDI   R18, (1<<4)|(1<<3)|(0x07)  ; Setup  watchdog to 0.5s (1<<WDCE)|(1<<WDE)|(101b) 
	  OUT   0x21, R18                  ; Enable change of watchdog parameters (WDTCR <- R18)
      OUT   0x21, R18                  ; Actual change of watchdog parameters (WDTCR <- R18)
