...
static uint8_t search_sensors(void)
{
	...
	uart1_puts_P( NEWLINESTR "Scanning Bus for DS18X20" NEWLINESTR );
	...
	while ( diff != OW_LAST_DEVICE && nSensors < MAXSENSORS ) {
		...
		if( diff == OW_PRESENCE_ERR ) {
			uart1_puts_P( "No Sensor found" NEWLINESTR );
			break;
		}
		
		if( diff == OW_DATA_ERR ) {
			uart1_puts_P( "Bus Error" NEWLINESTR );
			break;
		}
...
static void uart1_put_temp(int16_t decicelsius)
{
	char s[10];

	uart1_put_int( decicelsius );
	uart1_puts_P(" deci°C, ");
	DS18X20_format_from_decicelsius( decicelsius, s, 10 );
	uart1_puts( s );
	uart1_puts_P(" °C");
...
static void uart1_put_temp_maxres(int32_t tval)
{
	char s[10];

	uart1_put_longint( tval );
	uart1_puts_P(" °Ce-4, ");
	DS18X20_format_from_maxres( tval, s, 10 );
	uart1_puts( s );
	uart1_puts_P(" °C");
...
static void th_tl_dump(uint8_t *sp)
{
	...
	uart1_puts_P( "TH/TL in scratchpad of sensor 1 now : " );
	uart1_put_int( sp[DS18X20_TH_REG] );
	uart1_puts_P( " / " ); 
	uart1_put_int( sp[DS18X20_TL_REG] );
	uart1_puts_P( NEWLINESTR ); 
}
...
static void eeprom_test(void)
{
	...
	uart1_puts_P( NEWLINESTR "DS18x20 EEPROM support test for first sensor" NEWLINESTR ); 
	...
	uart1_puts_P( "TH+1 and TL+1 written to scratchpad" NEWLINESTR );
	th_tl_dump( sp );
	DS18X20_scratchpad_to_eeprom( DS18X20_POWER_PARASITE, &gSensorIDs[0][0] );
	uart1_puts_P( "scratchpad copied to DS18x20 EEPROM" NEWLINESTR );
	DS18X20_write_scratchpad( &gSensorIDs[0][0], 0, 0, DS18B20_12_BIT );
	uart1_puts_P( "TH and TL in scratchpad set to 0" NEWLINESTR );
	th_tl_dump( sp );
	DS18X20_eeprom_to_scratchpad(&gSensorIDs[0][0]);
	uart1_puts_P( "DS18x20 EEPROM copied back to scratchpad" NEWLINESTR );
	DS18X20_read_scratchpad( &gSensorIDs[0][0], sp, DS18X20_SP_SIZE );
	if ( ( th == sp[DS18X20_TH_REG] ) && ( tl == sp[DS18X20_TL_REG] ) ) {
		uart1_puts_P( "TH and TL verified" NEWLINESTR );
	} else {
		uart1_puts_P( "verify failed" NEWLINESTR );
	}
	th_tl_dump( sp );
}
...
int main( void )
{
  ...
  // Make PB0 an output
	...
	uart1_puts_P( NEWLINESTR "DS18X20 1-Wire-Reader Demo by Martin Thomas" NEWLINESTR );
	uart1_puts_P(            "-------------------------------------------" );
	
	nSensors = search_sensors();
	uart1_put_int( (int)nSensors );
	uart1_puts_P( " DS18X20 Sensor(s) available:" NEWLINESTR );
	

#if DS18X20_VERBOSE
	for (i = 0; i < nSensors; i++ ) {
		uart1_puts_P("# in Bus :");
		uart1_put_int( (int)i + 1);
		uart1_puts_P(" : ");
	...
	for ( i = 0; i < nSensors; i++ ) {
		...
		if ( gSensorIDs[i][0] == DS18S20_FAMILY_CODE ) {
			uart1_puts_P( "DS18S20/DS1820" );
		} else if ( gSensorIDs[i][0] == DS1822_FAMILY_CODE ) {
			uart1_puts_P( "DS1822" );
		}
		else {
			uart1_puts_P( "DS18B20" );
		}
		uart1_puts_P( " which is " );
		if ( DS18X20_get_power_status( &gSensorIDs[i][0] ) == DS18X20_POWER_PARASITE ) {
			uart1_puts_P( "parasite" );
		} else {
			uart1_puts_P( "externally" ); 
		}
		uart1_puts_P( " powered" NEWLINESTR );
	}
...
for(;;){
	if ( nSensors == 1 ) {
		uart1_puts_P( NEWLINESTR "There is only one sensor "
	...
	for(;;) {   // main loop
		...
		uart1_puts_P( NEWLINESTR "Convert_T and Read Sensor by Sensor (reverse order)" NEWLINESTR ); 
		for ( i = nSensors; i > 0; i-- ) {
			if ( DS18X20_start_meas( DS18X20_POWER_PARASITE, 
				...
				if ( DS18X20_read_decicelsius( &gSensorIDs[i-1][0], &decicelsius) 
					...
					uart1_puts_P( "CRC Error (lost connection?)" );
					error++;
				}
				uart1_puts_P( NEWLINESTR );
			}
			else {
				uart1_puts_P( "Start meas. failed (short circuit?)" );
				error++;
			}
		}
		
		uart1_puts_P( NEWLINESTR "Convert_T for all Sensors and Read Sensor by Sensor" NEWLINESTR );
		if ( DS18X20_start_meas( DS18X20_POWER_PARASITE, NULL ) 
			...
			for ( i = 0; i < nSensors; i++ ) {
				...
				if ( DS18X20_read_maxres( &gSensorIDs[i][0], &temp_eminus4 )
				     == DS18X20_OK ) {
					uart1_put_temp_maxres( temp_eminus4 );
				}
				else {
					uart1_puts_P( "CRC Error (lost connection?)" );
					error++;
...
#if DS18X20_VERBOSE
		// all devices:
		uart1_puts_P( NEWLINESTR "Verbose output" NEWLINESTR ); 
		DS18X20_start_meas( DS18X20_POWER_PARASITE, NULL );
		_delay_ms( DS18B20_TCONV_12BIT );
		...
		if ( error ) {
			uart1_puts_P( "*** problems - rescanning bus ..." );
			nSensors = search_sensors();
			uart1_put_int( (int) nSensors );
			uart1_puts_P( " DS18X20 Sensor(s) available" NEWLINESTR );
			error = 0;
