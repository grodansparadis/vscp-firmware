/*********************************************************************
 *
 *                  SNMP Module for Modtronix TCP/IP Stack
 *
 *********************************************************************
 * FileName:        SNMP.c
...
#define STRUCTURE               (0x30ul)
#define ASN_INT                 (0x02ul)
#define OCTET_STRING            (0x04ul)
#define ASN_NULL                (0x05ul)
#define ASN_OID                 (0x06ul)

// SNMP specific variables
#define SNMP_IP_ADDR            (0x40ul)
#define SNMP_COUNTER32          (0x41ul)
#define SNMP_GAUGE32            (0x42ul)
#define SNMP_TIME_TICKS         (0x43ul)
#define SNMP_OPAQUE             (0x44ul)
#define SNMP_NSAP_ADDR          (0x45ul)


#define GET_REQUEST             (0xa0ul)
#define GET_NEXT_REQUEST        (0xa1ul)
#define GET_RESPONSE            (0xa2ul)
#define SET_REQUEST             (0xa3ul)
#define TRAP                    (0xa4ul)
...
typedef union _SNMP_STATUS
{
    ...
    #define INVALID_UDP_SOCKET  (0xFFFFul)
    ...
    #undef MY_IP_BYTE3
    #undef MY_IP_BYTE4
    #define MY_IP_BYTE1             (10u)
    #define MY_IP_BYTE2             (10u)
    #define MY_IP_BYTE3             (5u)
    #define MY_IP_BYTE4             (15u)
...
static void ReadMIBRecord(FILE h, OID_INFO *rec);
...
void SNMPInit(void)
{
    ...
    SNMPAgentSocket = UDPOpen(SNMP_AGENT_PORT, 0, INVALID_UDP_PORT);
...
BOOL SNMPTask(void)
{
    ...
    hMIBFile = fileOpen((BYTE*)snmpBIBFile, 0);
    if ( hMIBFile != FILE_INVALID )
    {
 ...
 * @param notificaitonCode Notification Code to use
...
BOOL SNMPIsNotifyReady(IP_ADDR *remoteHost)
{
    ...
    if ( ARPIsResolved(remoteHost, &remoteNode.MACAddr) )
    {
        ...
        SNMPNotifyInfo.socket = UDPOpen(SNMP_AGENT_NOTIFY_PORT, &remoteNode, SNMP_NMS_PORT);
 ...
 *                  - Given SNMP_BIB_FILE does not exist in FILE
 *                  - Given var does not exist.
 *                  - Previously given agentID does not exist
 *                  - Data type of given var is unknown - only possible if FILE itself was corrupted.
 */
               ...
               SNMP_INDEX index)
{
    ...
    // Get complete OID string from FILE.
...
_GetNextLeaf:
    ...
    indexInfo.Val = fileGetByte();

    // Fetch index ID.
    indexRec.id = fileGetByte();
    // Fetch index data type.
    indexRec.dataType = fileGetByte();
...
static BOOL OIDLookup(BYTE *oid, BYTE oidLen, OID_INFO *rec)
{
    ...
    while( 1 )
    {
        ...
        // and child data.
        rec->hNode = fsysTell(); // hNode;

        // Read OID byte.
        tempOID = fileGetByte();

        // Read Node Info
        rec->nodeInfo.Val = fileGetByte();
        ...
        {
            tempData.v[0] = fileGetByte();
            tempData.v[1] = fileGetByte();
        ...
        if ( tempOID != *oid )
        {
            ...
            {
                fileGetByteEnd();
                hNode = fsysSeek((FILE)tempData.Val);
            }
            else
                goto DidNotFindIt;
        }
...
static BOOL GetNextLeaf(OID_INFO *n)
{
    ...
    while( 1 )
    {
        // Remember current FILE position for this node.
        n->hNode = fsysTell();

        fileGetByteBegin(n->hNode);
        ...
        if ( n->nodeInfo.Flags.bIsSibling ||
             n->nodeInfo.Flags.bIsDistantSibling )
        {
            temp.byte.LSB = fileGetByte();
            temp.byte.MSB = fileGetByte();
            n->hSibling = temp.Val;
        }
...
static void _SNMPDuplexInit(UDP_SOCKET socket)
{
    ...
    while( !UDPIsPutReady(socket) ) FAST_USER_PROCESS();
...
static void ReadMIBRecord(FILE h, OID_INFO *rec)
{
    ...
    {
        tempVal.byte.LSB = fileGetByte();
        tempVal.byte.MSB = fileGetByte();
